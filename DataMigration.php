...
class DataMigration 
{
	...
	function makechanges()
	{
		...
		$this->oldconn->query($sql_mickie1);
		...
		$this->oldconn->query($sql_mickie2);
		...
		$this->oldconn->query($sql_mickie3);
		...
		*/  
		...
		//Commented by Mickie to enter the currency entry in the field table
		/*
				...
				$reports = Array(Array('reportname'=>'Contacts by Accounts',
						...
						$advfilters = Array(
								...
								foreach($rptfolder as $key=>$rptarray)
								{
									...
									{
										$this->PopulateReportFolder($foldername,$folderdescription);
								...
								foreach($reports as $key=>$report)
								{
									...
									$this->insertReports($queryid,$reportid[$report['reportfolder']],$report['reportname'],$report['description'],$report['reporttype']);
									...
									if(isset($stdfilters[$report['stdfilterid']]))
									{
										...
										$this->insertStdFilter($queryid,$stdfilters[$i]['columnname'],$stdfilters[$i]['datefilter'],$stdfilters[$i]['startdate'],$stdfilters[$i]['enddate']);
									}
									...
									{
										$this->insertSortColumns($queryid,$sortorder[$report['sortid']]);
									}
								}
...
$cvadvfilters = Array(
		   ...
		   //Inserting into the custom view sequence table
			$sql1 = "insert into customview_seq values(0)";
		     ...
		     {
			     $queryid = $this->insertCustomView($customview['viewname'],$customview['setdefault'],$customview['setmetrics'],$customview['cvmodule']);
			     $this->insertCvColumns($queryid,$cvcolumns[$key]);

			     if(isset($cvstdfilters[$customview['stdfilterid']]))
			     {
				     $i = $customview['stdfilterid'];
				    $this->insertCvStdFilter($queryid,$cvstdfilters[$i]['columnname'],$cvstdfilters[$i]['datefilter'],$cvstdfilters[$i]['startdate'],$cvstdfilters[$i]['enddate']);
			     }
			     if(isset($cvadvfilters[$customview['advfilterid']]))
			     {
				     $this->insertCvAdvFilter($queryid,$cvadvfilters[$customview['advfilterid']]);
			     }
		     }
	...
	{
		$genQueryId = $this->oldconn->getUniqueID("selectquery");
		echo '<BR> inside insertSelectQuery '.$genQueryId.' <BR>';
		
		if($genQueryId != "")
		{
			$iquerysql = "insert into selectquery (QUERYID,STARTINDEX,NUMOFOBJECTS) values (".$genQueryId.",0,0)";
			//echo "<<<<<<<QUERY>>>>>>><br>".$iquerysql;
			$iquerysqlresult = $this->oldconn->query($iquerysql);
		}
	...
	function insertSelectColumns($queryid,$columnname)
	{
		...
		if($queryid != "")
		{
			for($i=0;$i < count($columnname);$i++)
			{
				...
				$icolumnsqlresult = $this->oldconn->query($icolumnsql);	
			}
		}
	}
	...
	function insertReports($queryid,$folderid,$reportname,$description,$reporttype)
	{
		...
		if($queryid != "")
		{
			...
			$ireportresult = $this->oldconn->query($ireportsql);
		}
	}
	...
	function insertReportModules($queryid,$primarymodule,$secondarymodule)
	{
		...
		if($queryid != "")
		{
			...
			$ireportmoduleresult = $this->oldconn->query($ireportmodulesql);
		}
	}
	...
	function insertSortColumns($queryid,$sortlists)
	{
		...
		if($queryid != "")
		{
			...
			{
				$sort_bysql = "insert into reportsortcol (SORTCOLID,REPORTID,COLUMNNAME,SORTORDER) values (".($i+1).",".$queryid.",'".$sort['columnname']."','".$sort['sortorder']."')";
				$sort_byresult = $this->oldconn->query($sort_bysql);
			}
		}
	...
	function insertAdvFilter($queryid,$filters)
	{
		...
		if($queryid != "")
		{
			foreach($filters as $i=>$filter)
			{
				...
				$irelcriteriaresult = $this->oldconn->query($irelcriteriasql);
			}
		}
	}
	...
	function insertCustomView($viewname,$setdefault,$setmetrics,$cvmodule)
	{
		...
		$genCVid = $this->oldconn->getUniqueID("customview");
		if($genCVid != "")
		{
			...
			//echo $customviewsql;
			$customviewresult = $this->oldconn->query($customviewsql);
		}
	...
	function insertCvColumns($CVid,$columnslist)
	{
		...
		if($CVid != "")
		{
			for($i=0;$i<count($columnslist);$i++)
			{
				...
				//echo $columnsql;
				$columnresult = $this->oldconn->query($columnsql);
			}
		}
	}
	...
	function insertCvStdFilter($CVid,$filtercolumn,$filtercriteria,$startdate,$enddate)
	{
		...
		if($CVid != "")
		{
			...
			//echo $stdfiltersql;
			$stdfilterresult = $this->oldconn->query($stdfiltersql);
		}
	}
	...
	function insertCvAdvFilter($CVid,$filters)
	{
		...
		if($CVid != "")
		{
			foreach($filters as $i=>$filter)
			{
				...
				//echo $advfiltersql;
				$advfilterresult = $this->oldconn->query($advfiltersql);
			}
