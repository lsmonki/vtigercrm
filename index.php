 ...
 ********************************************************************************/
/*********************************************************************************
 * $Header:  vtiger_crm/index.php,v 1.6 2004/08/27 07:07:25 don Exp $
...
if(isset($action) && isset($module))
{
	...
	$log->debug("in $action");
	if(ereg("^Save", $action) || ereg("^Delete", $action) || ereg("^Popup", $action) || ereg("^ChangePassword", $action) || ereg("^Authenticate", $action) || ereg("^Logout", $action))
	{
		$skipHeaders=true;
...
echo("<font size=1><br>&copy; 2003-4 vtiger.com, with components from Apache, MySQL, PHP and SugarCRM, under separate Open Source licenses. See <a href='http://vtiger.com/copyrights/LICENSE_AGREEMENT.txt'>Copyrights </a> for details. </font><br>\n");
