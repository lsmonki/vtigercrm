...
$org_separator = "<br>&nbsp;";
$local_log =& LoggerManager::getLogger('OrganizationAjax');
$my_status = "Failure";
...
if($ajaxaction == "DETAILVIEW")
{
    ...
    if( $uitype != '' && $module == "Organization") {

	$focus = new $module;
	$focus->id = $id;

	// The organization table consist of text fields and
	// one image ...
	if( $uitype == 107) {
	    if( isset( $_FILES[$fieldname])) {
		$focus->uploadAndSaveFile($_FILES[$fieldname]);
		$my_status = 'success';
	    }
	}

	// The text fields otherwise
	else {
	    $sql = "UPDATE ".$tablename." SET ".$fieldname."='".$fieldvalue."'
		WHERE organizationname='".$id."'";
	    $adb->query( $sql);
	    $my_status = 'success';
	}
    }
