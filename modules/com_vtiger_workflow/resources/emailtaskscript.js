function VTEmailTask($){
	...
	function getDescribeObjects(moduleName, callback){
		vtinst.describeObject(moduleName, handleError(function(result){
			...
			var relatedModules = reduceR(union, referenceFieldModules,
										 [parent['name']]);
	...
	function fillSelectBox(id, modules, parentModule, filterPred){
		...
		function filteredFields(fields){
			return filter(
				function(e){
					var fieldCheck = !contains(['autogenerated', 'reference', 'owner'],
																		 e.type.name);
		...
		function fieldReferenceNames(referenceField){
			...
			return reduceR(concat,
								   map(forModule,
									   referenceField['type']['refersTo']),[]);
		}

		var referenceFields = reduceR(concat,
														map(fieldReferenceNames,
														referenceFieldTypes), []);
		var fieldLabels = dict(parentFields.concat(referenceFields));
		var select = $('#'+id);
		var optionClass = id+'_option';
		$.each(fieldLabels, function(k, v){
			select.append('<option class="'+optionClass+'" '+
										'value="'+k+'">' + v + '</option>');
		});
