...
echo '11111       '.$use_imap_tls;
echo '<br>';
echo '22222222    '.$imapServerAddress;
echo '<br>';
echo '33333333       '.$imapPort;
echo '<br>';
$stream = fsockopen( ($use_imap_tls?'tls://':'').$imapServerAddress, $imapPort,$errorNumber, $errorString);
if(!$stream) {
    do_err("Error connecting to IMAP server \"$imapServerAddress:$imapPort\"."."Server error: (".$errorNumber.") ". htmlspecialchars($errorString));
}
