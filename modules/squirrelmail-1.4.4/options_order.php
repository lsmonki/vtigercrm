   ...
   html_tag( 'td', '', 'center', $color[4] );
    ...
    if (count($index_order))
    {
        ...
        for ($i=1; $i <= count($index_order); $i++) {
            $tmp = $index_order[$i];
            echo html_tag( 'tr' );
            echo html_tag( 'td', '<small><a href="index.php?module=squirrelmail-1.4.4&action=options_order&method=up&amp;num=' . $i . '">'. _("up") .'</a></small>' );
            echo html_tag( 'td', '<small>&nbsp;|&nbsp;</small>' );
            echo html_tag( 'td', '<small><a href="index.php?module=squirrelmail-1.4.4&action=options_order&method=down&amp;num=' . $i . '">'. _("down") .'</a></small>' );
            echo html_tag( 'td', '<small>&nbsp;|&nbsp;</small>' );
            echo html_tag( 'td' );
            /* Always show the subject */
            if ($tmp != 4)
               echo '<small><a href="index.php?module=squirrelmail-1.4.4&action=options_order&method=remove&amp;num=' . $i . '">' . _("remove") . '</a></small>';
            else
               echo '&nbsp;'; 
            echo '</td>';
            echo html_tag( 'td', '<small>&nbsp;-&nbsp;</small>' );
    ...
    if (count($index_order) != count($available)) {
        echo '<form name="f" method="post" action="index.php?module=squirrelmail-1.4.4&action=options_order">';
    ...
    echo html_tag( 'p', '<a href="index.php?module=squirrelmail-1.4.4&action=options">' . _("Return to options page") . '</a></p><br />' );
