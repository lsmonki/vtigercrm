...
//        $body .= '<center><small><a href="index.php?module=squirrelmail-1.4.4&action=download&absolute_dl=true&amp;' .
  //               $link . '">' . _("Download this as a file") .  '</a>';
        ...
        if($text != '') {
            $body .= '&nbsp;|&nbsp;<a href="index.php?module=squirrelmail-1.4.4&action=read_body&' . $link . '">' . $text . '</a>';
        }
...
function formatVTigerBody($imap_stream, $message,$ent_num,$id, $mailbox='INBOX') {
    /* This if statement checks for the entity to show as the
     * primary message. To add more of them, just put them in the
     * order that is their priority.
     */
...
function formatAttachments($message, $exclude_id, $mailbox, $id) {
    ...
    foreach ($att_ar as $att) {
        ...
        $links['download link']['href'] = "index.php?module=squirrelmail-1.4.4&action=download&absolute_dl=true&amp;passed_id=$id&amp;mailbox=$urlMailbox&amp;ent_id=$ent";
        $ImageURL = '';
        if ($type0 =='message' && $type1 == 'rfc822') {
            $default_page = 'index.php?module=squirrelmail-1.4.4&action=read_body';
            ...
            $default_page = 'index.php?module=squirrelmail-1.4.4&action=download';
...
function sq_cid2http($message, $id, $cidurl, $mailbox){
    ...
    $httpurl = '';
    if ($linkurl) {
        $httpurl = $quotchar . 'index.php?module=squirrelmail-1.4.4&action=download&absolute_dl=true&amp;' .
