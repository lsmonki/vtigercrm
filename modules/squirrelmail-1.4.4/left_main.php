...
function formatMailboxName($imapConnection, $box_array) {
    ...
    if ($status !== false) {
        $line .= '<a href="index.php?module=squirrelmail-1.4.4&action=right_main&PG_SHOWALL=0&amp;sort=0&amp;startMessage=1&amp;mailbox='.
    ...
    if (($move_to_trash) && ($real_box == $trash_folder)) {
        ...
        if (($numMessages > 0) or ($box_array['parent'] == 1)) {
            ...
            $line .= "\n<small>\n" .
                    '&nbsp;&nbsp;(<a href="index.php?module=squirrelmail-1.4.4&action=empty_trash" style="text-decoration:none">'._("Purge").'</a>)' .
                    '</small>';
        }
    } else {
