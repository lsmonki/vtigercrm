...
//The problem is to pass the msgData to the next page when one makes a response to the mail. Now, the message data must be prepopulated into the TextArea of the EditView page as a result of clicking either of the links
    ...
    $s .= '<a href="index.php?module=Emails&action=EditView'.$modifiedcomp_uri.'&msg_to='.$msgvtTo.'&mg_subject='.$msgvtSubject.'&msg_id_num='.$passed_id.'&passData=true'.'">'.$mod_strings['LNK_REPLY'].'</a>';
    // echo $string;
    ...
    $s .= '<a href="index.php?module=Emails&action=EditView'.$modifiedcomp_uri.'&msg_to='.$msgvtTo.'&msg_cc='.$msgvtcc.'&mg_subject='.$msgvtSubject.'&msg_id_num='.$passed_id.'&passData=true'.'">'.$mod_strings['LNK_REPLY_ALL'].'</a>';
    $s .= '</small></td></tr></table>';
