...
//        $s .= '<a href="'.$url.'">'._("View Message").'</a>';
   ...
   // echo $comp_action_uri;
    ...
    $s .= '<a href="index.php?module=Emails&action=EditView'.$modifiedcomp_uri.'&msg_to='.$msgvtTo.'&mg_subject='.$msgvtSubject.'">'.$mod_strings['LNK_REPLY'].'</a>';
    // echo $string;

    $comp_action_uri = $modifiedcomp_uri . '&amp;smaction=reply_all';
    $s .= $topbar_delimiter;
    //$s .= makeComposeLink($comp_action_uri, _("Reply All"));
    $s .= '<a href="index.php?module=Emails&action=EditView'.$modifiedcomp_uri.'&msg_to='.$msgvtTo.'&msg_cc='.$msgvtcc.'&mg_subject='.$msgvtSubject.'">'.$mod_strings['LNK_REPLY_ALL'].'</a>';
    $s .= '</small></td></tr></table>';
