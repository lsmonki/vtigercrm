...
$modval=$_REQUEST['modname'];
$dash=$_REQUEST['dash'];
if(isset($modval) || $modval !="")
{
...
if(isset($_REQUEST['primecvid']) && $_REQUEST['primecvid']!="")
{
...
if((isset($_REQUEST['showmaxval']) || $_REQUEST['showmaxval']!="") && (isset($_REQUEST['sid']) || $_REQUEST['sid']!=""))
{
	$sid=$_REQUEST['sid'];
...
if(isset($_REQUEST['dashVal']) || $_REQUEST['dashVal']!="")
{
	$did=$_REQUEST['did'];
	global $adb;
...
if(isset($_REQUEST['matrixsequence']) || $_REQUEST['matrixsequence'] != "")
{
	global $adb;
	...
	{
		$cursequence = $i +1;
		$query = 'update vtiger_homestuff set stuffsequence='.$cursequence.' where stuffid='.$sequence[$i];
		$result = $adb->query($query);
	}
...
if(isset($_REQUEST['act']) || $_REQUEST['act'] =="hide")
{
	...
	$res=$adb->query($qry);
	updateUserTable($current_user->id);
	echo "SUCCESS";
}
...
{
	global $adb;
	$query = "select hometype from vtiger_homedefault inner join vtiger_homestuff on vtiger_homedefault.stuffid=vtiger_homestuff.stuffid where userid=$userid and visible=0";
	$res=$adb->query($query);
	$values = Array();
	for($i=0; $i< $adb->num_rows($res);$i++)
	{
		$values[] = $adb->query_result($res,$i,"hometype");
	}
	$insertvalues = implode(',',$values);
	$query = "update vtiger_users set homeorder='$insertvalues' where id=$userid";
	$res=$adb->query($query);
}
?>