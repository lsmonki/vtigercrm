 ...
 *                                support_posting.php
 *                            -------------------
 *   begin                : Saturday, Mar 26, 2004
 *   copyright            : (C) 2001 The phpBB Group
 *   email                : support@phpbb.com
 *
 *   $Id: support_posting.php,v 1.9 2005/01/04 15:04:12 saraj Exp $
 *
 *
 ***************************************************************************/
...
switch( $mode )
{
	...
	case 'editpost':
		...
		break;
	case 'update_ticket':
		$ticket_id = ( !empty($HTTP_GET_VARS['ticket_id']) ) ? $HTTP_GET_VARS['ticket_id'] : $HTTP_POST_VARS['ticket_id'];
		$sql = "UPDATE " . TOPICS_TABLE . " SET ticket_id = '" . $ticket_id . "' WHERE topic_id = " . $topic_id ." OR topic_moved_id = ".$topic_id;
                        if ( !$db->sql_query($sql) )
                        {
                                message_die(GENERAL_ERROR, 'Could not update ticket id for topic '.$topic_id, '', __LINE__, __FILE__, $sql);
                        }
	        message_die(GENERAL_MESSAGE, "Ticket ID: " . $ticket_id . " has been updated for topic $topic_id.");
		break;
	case 'delete':
	case 'poll_delete':
...
if ( $result = $db->sql_query($sql) )
{
	...
	$forum_email = $post_info['forum_email'];
...
$blogcat='';
	...
	switch ( $mode )
	{
       ...
       if ( $error_msg == '' )
       {
		...
		case 'reply':
			$sql = "select u.username,u.user_id from " . USERS_TABLE . " u where u.user_email='".$forum_email."'";
			if ( $result = $db->sql_query($sql) )
			{
			        $post_info = $db->sql_fetchrow($result);
			        $username = $post_info['username'];
			        $userdata['user_id'] = $post_info['user_id'];
				$userdata['session_logged_in']=1;
				$userdata['username']=$post_info['username'];
			}
			else
				$username = $forum_email;

			$subject = ( !empty($HTTP_POST_VARS['subject']) ) ? trim($HTTP_POST_VARS['subject']) : trim($HTTP_GET_VARS['subject']);
			$message = ( !empty($HTTP_POST_VARS['message']) ) ? $HTTP_POST_VARS['message'] : $HTTP_GET_VARS['message'];
			$poll_title = ( isset($HTTP_POST_VARS['poll_title']) && $is_auth['auth_pollcreate'] ) ? $HTTP_POST_VARS['poll_title'] : '';
			$poll_options = ( isset($HTTP_POST_VARS['poll_option_text']) && $is_auth['auth_pollcreate'] ) ? $HTTP_POST_VARS['poll_option_text'] : '';
			$poll_length = ( isset($HTTP_POST_VARS['poll_length']) && $is_auth['auth_pollcreate'] ) ? $HTTP_POST_VARS['poll_length'] : '';
			$bbcode_uid = '';
