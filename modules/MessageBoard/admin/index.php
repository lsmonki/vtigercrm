 ...
 *
 *   $Id: index.php,v 1.9 2005/01/04 15:04:14 saraj Exp $
 *
 *
 ***************************************************************************/
...
if( isset($HTTP_GET_VARS['pane']) && $HTTP_GET_VARS['pane'] == 'left' )
{
  ...
  $dir = @opendir($phpbb_root_path ."admin/");
	...
	$template->assign_vars(array(
		"U_FORUM_INDEX" => append_sid($modifiedphp_root ."index"),
		"U_ADMIN_INDEX" => append_sid($modifiedphp_root . "index&pane=right"),
		"U_CRM_INDEX" => append_sid("../../../index.php?action=index&module=Home"),

		"L_FORUM_INDEX" => $lang['Main_index'],
		"L_ADMIN_INDEX" => $lang['Admin_Index'],
		"L_VTIGER_CRM" => $lang['CRM_Index'])
	);
	
	ksort($forum_module);

	while( list($cat, $action_array) = each($forum_module) )
	{
  ...
  include($phpbb_root_path . 'admin/page_header_admin.'.$phpEx);
	...
	if( count($onlinerow_reg) )
	{
		...
		for($i = 0; $i < count($onlinerow_reg); $i++)
		{
			if( !inarray($onlinerow_reg[$i]['user_id'], $reg_userid_ary) )
			{
				...
				if( $onlinerow_reg[$i]['user_session_page'] < 1 )
				{
					switch($onlinerow_reg[$i]['user_session_page'])
					{
						...
						case PAGE_FAQ:
							...
							$location_url = "index.$phpEx?pane=right";
							break;
						case PAGE_WEBLOG:                 //Begin: added by isaac for weblogs
						...
						case PAGE_WEBLOG_ENTRY:
							...
							$location_url = "index.$phpEx?pane=right";
							break;                        //End: by isaac for weblogs
						default:
							$location = $lang['Forum_index'];
							$location_url = "index.$phpEx?pane=right";
					}
				...
				{
					$location_url = append_sid($modifiedphp_root . "admin_forums&mode=editforum&amp;" . POST_FORUM_URL . "=" . $onlinerow_reg[$i]['user_session_page']);
				...
				$template->assign_block_vars("reg_user_row", array(
					...
					"U_WHOIS_IP" => "http://www.dshield.org/ipinfo.php?ip=$reg_ip",
					"U_USER_PROFILE" => append_sid($modifiedphp_root . "admin_users&mode=edit&amp;" . POST_USERS_URL . "=" . $onlinerow_reg[$i]['user_id']),
					"U_FORUM_LOCATION" => append_sid($modifiedphp_root .$location_url))
				);
			}
		}
	...
	if( count($onlinerow_guest) )
	{
		...
		for($i = 0; $i < count($onlinerow_guest); $i++)
		{
			...
			if( $onlinerow_guest[$i]['session_page'] < 1 )
			{
				switch( $onlinerow_guest[$i]['session_page'] )
				{
					...
					case PAGE_FAQ:
						...
						$location_url = "index.$phpEx?pane=right";
						break;
					case PAGE_WEBLOG:                  //added by Isaac for weblogs
						$location = $lang['Viewing_Weblog'];
						$location_url = "index.$phpEx?pane=right";
						break;
					case PAGE_WEBLOG2:
			...
			{
				$location_url = append_sid($modifiedphp_root . "admin_forums&mode=editforum&amp;" . POST_FORUM_URL . "=" . $onlinerow_guest[$i]['session_page']);
			...
			$template->assign_block_vars("guest_user_row", array(
				...
				"U_WHOIS_IP" => "http://www.dshield.org/ipinfo.php?ip=$guest_ip",
...
else
{
  ...
  //
	...
	$template->assign_vars(array(
		"S_FRAME_NAV" => append_sid($modifiedphp_root .  "admin/index&pane=left"),
		"S_FRAME_MAIN" => append_sid($modifiedphp_root ."admin/index&pane=right"))
	);
