 ...
 *
 *   $Id: admin_forums.php,v 1.9 2005/01/04 15:04:13 saraj Exp $
 *
 ***************************************************************************/
...
if( !empty($setmodules) )
{
	...
	$forum_module['Forums']['Manage'] = $file;
	return;
}
...
$sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, forum_name, cat_id, forum_desc, forum_order, forum_status, forum_notify,

prune_enable" . $field_sql . ")
	VALUES ('" . $next_id . "', '" . str_replace("\'", "''", $HTTP_POST_VARS['forumname']) . "', " .

intval($HTTP_POST_VARS[POST_CAT_URL]) . ", '" . str_replace("\'", "''", $HTTP_POST_VARS['forumdesc']) . "', $next_order, " .

intval($HTTP_POST_VARS['forumstatus']) . ", " . intval($HTTP_POST_VARS['notify_enable']) . ", " .
...
intval($HTTP_POST_VARS[POST_CAT_URL]) . ", forum_desc = '" . str_replace("\'", "''", $HTTP_POST_VARS['forumdesc']) . "',

forum_status = " . intval($HTTP_POST_VARS['forumstatus']) . ", forum_notify = " . intval($HTTP_POST_VARS['notify_enable']) .
	...
	if( !$result = $db->sql_query($sql) )
	{
		...
		case 'modcat':
			...
			$sql = "UPDATE " . CATEGORIES_TABLE . "
				SET cat_title = '" . str_replace("\'", "''", $HTTP_POST_VARS['cat_title']) . "', include_on_index = $include_on_index
				WHERE cat_id = " . intval($HTTP_POST_VARS[POST_CAT_URL]);
			if( !$result = $db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, "Couldn't update forum information", "", __LINE__, __FILE__, $sql);
...
if( $total_categories = $db->sql_numrows($q_categories) )
{
	...
	for($i = 0; $i < $total_categories; $i++)
	{
		...
		$template->assign_block_vars("catrow", array(
			...
			'U_VIEWCAT' => append_sid($modifiedphp_root."index&" . POST_CAT_URL . "=$cat_id"))
		);
		...
		for($j = 0; $j < $total_forums; $j++)
		{
			...
			if ($forum_rows[$j]['cat_id'] == $cat_id)
			{
				...
				$template->assign_block_vars("catrow.forumrow",	array(
					...
					'U_VIEWFORUM' => append_sid($modifiedphp_root ."viewforum&" . POST_FORUM_URL . "=$forum_id"),
