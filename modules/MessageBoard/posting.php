 ...
 *
 *   $Id: posting.php,v 1.9 2005/01/04 15:04:12 saraj Exp $
 *
 *
 ***************************************************************************/
...
// getting post or get variable value of cc_mail.
$cc_mail = ( !empty($HTTP_POST_VARS['cc_mail']) ) ? $HTTP_POST_VARS['cc_mail'] : $HTTP_GET_VARS['cc_mail'];
...
if ( isset($HTTP_POST_VARS['cancel']) )
{
	if ( $post_id )
	{
		$redirect = $modifiedphp_root. "viewtopic&" . POST_POST_URL . "=$post_id";
		$post_append = "#$post_id";
	}
	else if ( $topic_id )
	{
		$redirect = $modifiedphp_root . "viewtopic&" . POST_TOPIC_URL . "=$topic_id";
		$post_append = '';
	}
	else if ( $forum_id )
	{
		$redirect = $modifiedphp_root . "viewforum&" . POST_FORUM_URL . "=$forum_id";
		$post_append = '';
	}
	else
	{
		$redirect = $modifiedphp_root ."index";
		$post_append = '';
	}
...
if ( $result = $db->sql_query($sql) )
{
	...
	//added by isaac for cc mail feature
	if ($mode == 'newtopic' &&  ( $submit || $confirm ))
    {
        $topic_title =( !empty($HTTP_POST_VARS['subject']) ) ? trim($HTTP_POST_VARS['subject']) : '';
    }
    else
    {
	...
	if ( $mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete' )
	{
		...
		if ( $post_info['poster_id'] != $userdata['user_id'] && !$is_auth['auth_mod'] )
		{
			...
			$message .= '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid($modifiedphp_root ."viewtopic&" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
...
if ( !$is_auth[$is_auth_type] )
{
	...
	{
		case 'newtopic':
			$redirect = $modifiedphp_root ."mode=newtopic&" . POST_FORUM_URL . "=" . $forum_id;
			break;
		case 'reply':
		case 'topicreview':
			$redirect = $modifiedphp_root ."mode=reply&" . POST_TOPIC_URL . "=" . $topic_id;
			break;
		case 'quote':
		case 'editpost':
			$redirect = $modifiedphp_root . "mode=quote&" . POST_POST_URL ."=" . $post_id;
			break;
	}

	redirect(append_sid($modifiedphp_root ."login&redirect=posting&" . $redirect, true));
}
...
if ( ( $delete || $poll_delete || $mode == 'delete' ) && !$confirm )
{
	...
	$template->assign_vars(array(
		...
		'S_CONFIRM_ACTION' => append_sid($modifiedphp_root ."posting"),
...
else if ( $mode == 'vote' )
{
	...
	if ( !empty($HTTP_POST_VARS['vote_id']) )
	{
		...
		// Added for "Post As Public" feature by Isaac
		if (!isset($HTTP_POST_VARS['post_public']))
		{
			$template->assign_vars(array(
				'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid($modifiedphp_root ."viewtopic&" . POST_TOPIC_URL . "=$topic_id") . '">')
			);
			$message .=  '<br /><br />' . sprintf($lang['Click_view_message'], '<a href="' . append_sid($modifiedphp_root ."viewtopic&" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
			message_die(GENERAL_MESSAGE, $message);
	   }
	   else
	   {
		   $template->assign_vars(array(
				'META' => '<meta http-equiv="refresh" content="3;url=' . append_sid($modifiedphp_root ."modcp&" . POST_TOPIC_URL . "=$topic_id&mode=move") . '">')
			);
...
$blogcat='';
	...
	switch ( $mode )
	{
        ...
        $bbcode_uid = '';
        $post_public = ( isset($HTTP_POST_VARS['post_public']) ) ? TRUE : FALSE;    //added by isaac
		...
		if ($error_msg == '' && $mode != 'poll_delete')
		{
			...
			if ( $post_data['first_post'] )
			{
				...
				if ( $topic_info = $db->sql_fetchrow($result) )
				{
					...
					if ($cc_mail != '')
					{
						//commented to remove auto completion of mail feature - Isaac
						//message_die(GENERAL_ERROR, 'The cc_mail is', '', __LINE__, __FILE__,$cc_mail);
				cc_to_users($mode, $post_data, $topic_title, $forum_id, $topic_id, $post_id, $cc_mail);
          		}
				}
			...
			else
			{
				...
				if ($cc_mail != '')
                {
	              //commented to remove auto completion of mail address feature - Isaac
	               /* $cc_mail = explode(",", $cc_mail);
                   $mailaddr = "";
                    foreach($cc_mail as $value)
                    {
                        $position =strpos($value,'@');
                        if ($position === false)
                        {
                           $mailaddr = "$mailaddr" .  "$value" . "@vtiger.com,";
                        }
                        else
                        {
                    ...
                    $mailaddr=substr($mailaddr,0,strlen($mailaddr) -1);
                    $cc_mail = $mailaddr; */
		        ...
		        sendto_support($mode, $post_data, $post_info['topic_title'], $forum_id, $topic_id, $post_id);
		}
...
//
// Post as  Public checkbox - show only if it is private forum - Isaac
//
if ($post_info['auth_read'] == 9)
//if ($is_auth['auth_post'])
{
	if ( $mode != 'editpost' || ( $mode == 'editpost' && $post_info['poster_id'] != ANONYMOUS ) )
	{
		$template->assign_block_vars('switch_post_public_checkbox', array());
	}
}
...
//
// List Forums in a drop-down list- Isaac
//
$sql = "SELECT c.cat_title, c.cat_id, f.forum_name, f.forum_id, f.auth_view
	FROM " . CATEGORIES_TABLE . " c, " . FORUMS_TABLE . " f
	WHERE f.cat_id = c.cat_id AND f.auth_view='0' AND f.auth_post=1 AND f.auth_reply=1
...
while( $row = $db->sql_fetchrow($result) )
{
	...
	{
		$move_forums .= '<option value="' . $row['forum_id'] . '">' . $row['forum_name'] . '</option>';
...
$EMBB_keys = array('','d') ;
$EMBB_widths = array('','55') ;
$EMBB_values = array('','Google') ;
...
$template->assign_vars(array(
	...
	'BBCODE_STATUS' => sprintf($bbcode_status, '<a href="' . append_sid($modifiedphp_root ."faq&mode=bbcode") . '" target="_phpbbcode">', '</a>'),
	...
	'L_POST_PUBLIC' => $lang['Post_Public'],  //added by isaac
	'L_POST_PUBLIC_MESSAGE' => $lang['Post_Public_Message'],        //added by isaac
	'L_SUPPORT_MAIL' => $lang['Support_Mail'],        //added by isaac
	'L_SUPPORT_MESSAGE' => $lang['Support_Message'],        //added by isaac
	...
	'U_VIEWTOPIC' => ( $mode == 'reply' ) ? append_sid($modifiedphp_root ."viewtopic&" . POST_TOPIC_URL . "=$topic_id&amp;postorder=desc") : '',
	'U_REVIEW_TOPIC' => ( $mode == 'reply' ) ? append_sid($modifiedphp_root ."posting&mode=topicreview&amp;" . POST_TOPIC_URL . "=$topic_id") : '',
	...
	'S_POST_PUBLIC_CHECKED' => ( $post_public ) ? 'checked="checked"' : '',   //added by isaac
	'S_TOPIC_ID' => $topic_id,
	'S_POST_ACTION' => append_sid($modifiedphp_root ."posting"),
	'S_MOVE_FORUMS' => $move_forums,                                             //added by isaac
