 ...
 *
 *   $Id: privmsg.php,v 1.9 2005/01/04 15:04:12 saraj Exp $
 *
 *
 ***************************************************************************/
...
else if ( $mode == 'read' )
{
	...
	$temp_url = append_sid($modifiedphp_root . "profile&mode=viewprofile&amp;" . POST_USERS_URL . '=' . $user_id_from);
	$profile_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_profile'] . '" alt="' . $lang['Read_profile'] . '" title="' . $lang['Read_profile'] . '" border="0" /></a>';
	$profile = '<a href="' . $temp_url . '">' . $lang['Read_profile'] . '</a>';

	$temp_url = append_sid("privmsg&mode=post&amp;" . POST_USERS_URL . "=$poster_id");
	$pm_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_pm'] . '" alt="' . $lang['Send_private_message'] . '" title="' . $lang['Send_private_message'] . '" border="0" /></a>';
	$pm = '<a href="' . $temp_url . '">' . $lang['Send_private_message'] . '</a>';

	if ( !empty($privmsg['user_viewemail']) || $userdata['user_level'] == ADMIN )
	{
		$email_uri = ( $board_config['board_email_form'] ) ? append_sid($modifiedphp_root . "profile&mode=email&amp;" . POST_USERS_URL .'=' . $user_id_from) : 'mailto:' . $privmsg['user_email'];
...
else if ( $submit || $refresh || $mode != '' )
{
	...
	$template->assign_vars(array(
		...
		'U_SEARCH_USER' => append_sid($modifiedphp_root . "search&mode=searchuser"),
		'U_VIEW_FORUM' => append_sid($modifiedphp_root . "privmsg"))
	);
...
if ( $row = $db->sql_fetchrow($result) )
{
	do
	{
		...
		$u_from_user_profile = append_sid($modifiedphp_root . "profile&mode=viewprofile&amp;" . POST_USERS_URL . "=$msg_userid");
		...
		$u_subject = append_sid($modifiedphp_root . "privmsg&folder=$folder&amp;mode=read&amp;" . POST_POST_URL . "=$privmsg_id");
