 ...
 *
 *   $Id: weblog_search.php,v 1.9 2005/01/04 15:04:13 saraj Exp $
 *
 *
 ***************************************************************************/
			...
			if ( $search_id == 'newposts' )
			{
				if ( $userdata['session_logged_in'] )
				{
					$sql = "SELECT post_id
						...
						WHERE p.forum_id=f.forum_id AND post_time >= " . (($search_time)? $search_time." AND post_time >= ":'' ).$userdata['user_lastvisit'];
				}
				else
				{
...
// MOD END - TOPICS I'VE STARTED - deejay
	...
	if ( $search_results != '' )
	{
		...
		$sql .= " AND f.forum_id = u.user_weblog ORDER BY ";
		switch ( $sort_by )
		{
			case 1:
		...
		//For actual search result count
		if ( !$result = $db->sql_query($search_cnt_qry) )
		{
			message_die(GENERAL_ERROR, 'Could not obtain search results', '', __LINE__, __FILE__, $sql);
		}
		while( $row = $db->sql_fetchrow($result) )
		...
		$total_match_count = count($searchcountset);
		...
		{
			$template->set_filenames(array(
				'body' => 'weblog_search_results_topics.tpl')
			);
		}
		...
		{
			$forum_url = append_sid("weblog.$phpEx?" . POST_WEBLOG_URL . '=' . $searchset[$i]['forum_id']);
			$topic_url = append_sid("weblog_entry.$phpEx?" . POST_ENTRY_URL . '=' . $searchset[$i]['topic_id'] . "&amp;highlight=$highlight_active");
			$post_url = append_sid("weblog_entry.$phpEx?" . POST_POST_URL . '=' . $searchset[$i]['post_id'] . "&amp;highlight=$highlight_active") . '#' . $searchset[$i]['post_id'];
			...
			else
			{
				...
				if ( ( $replies + 1 ) > $board_config['posts_per_page'] )
				{
					...
					{
						$goto_page .= '<a href="' . append_sid("weblog_entry.$phpEx?" . POST_ENTRY_URL . "=" . $topic_id . "&amp;start=$j") . '">' . $times . '</a>';
						...
						if ( $searchset[$i]['post_time'] > $userdata['user_lastvisit'] )
						{
							if ( !empty($tracking_topics) || !empty($tracking_forums) || isset($HTTP_COOKIE_VARS[$board_config['cookie_name'] . '_f_all']) )
							{
								...
								if ( $unread_topics )
								{
									...
									$newest_post_img = '<a href="' . append_sid("weblog_entry.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;view=newest") . '"><img src="' . $images['icon_newest_reply'] . '" alt="' . $lang['View_newest_post'] . '" title="' . $lang['View_newest_post'] . '" border="0" /></a> ';
								}
								else
								{
							...
							else if ( $searchset[$i]['post_time'] > $userdata['user_lastvisit'] )
							{
								...
								$newest_post_img = '<a href="' . append_sid("weblog_entry.$phpEx?" . POST_ENTRY_URL . "=$topic_id&amp;view=newest") . '"><img src="' . $images['icon_newest_reply'] . '" alt="' . $lang['View_newest_post'] . '" title="' . $lang['View_newest_post'] . '" border="0" /></a> ';
							}
							else
							{
								$folder_image = $folder;
				...
				$last_post_url = '<a href="' . append_sid("weblog_entry.$phpEx?"  . POST_ENTRY_URL . '=' . $searchset[$i]['topic_last_post_id']) . '#' . $searchset[$i]['topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" border="0" /></a>';
		...
		$base_url = "weblog_search.$phpEx?search_id=$search_id";
...
$template->assign_vars(array(
	...
	'S_SEARCH_ACTION' => append_sid("weblog_search.$phpEx?mode=results"),
