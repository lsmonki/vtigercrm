...
function phpbb_fetch_posts($forum_id = null, $fetch_mode = POSTS_FETCH_FIRST)
{
	...
	//AND f.cat_id  <> 255555   //Added By David For Sorting:
	...
	if ($CFG['topic_limit'] !='')
    	{
		echo $CFG['topic_limit'];
	      //$sql .= '
		//LIMIT 0,'.$CFG['topic_limit'];
    	}
    //end David
...
*/
	...
	$sql = "SELECT count(post_id) As total
		...
		WHERE p.forum_id=f.forum_id AND f.cat_id <> 26 AND post_time >= " . (($search_time)? $search_time." AND post_time >= ":'' ).$userdata['user_lastvisit'];
