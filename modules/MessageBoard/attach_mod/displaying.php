...
{
	global $template, $upload_dir, $userdata, $allowed_extensions, $display_categories, $download_modes, $db, $lang, $phpEx, $attachments, $upload_icons, $attach_config, $modifiedphp_root;
	...
	for ($i = 0; $i < $num_attachments; $i++)
	{
		...
		if (!$denied)
		{
			...
			if ($image)
			{
				...
				{
					$img_source = append_sid($modifiedphp_root . 'download&id=' . $attachments['_' . $post_id][$i]['attach_id']);
					$download_link = TRUE;
				}
				else
				{
			...
			if ($thumbnail)
			{
				...
				if ( (intval($attach_config['allow_ftp_upload'])) && (trim($attach_config['download_path']) == '') )
				{
					$thumb_source = append_sid($modifiedphp_root . 'download&id=' . $attachments['_' . $post_id][$i]['attach_id'] . '&thumb=1');
				}
				else
				{
				...
				$template->assign_block_vars('postrow.attach.cat_thumb_images', array(
					...
					'IMG_SRC' => append_sid($modifiedphp_root .'download&id=' . $attachments['_' . $post_id][$i]['attach_id']),
...
//					'U_DOWNLOAD_LINK' => append_sid('download.' . $phpEx . '?id=' . $attachments['_' . $post_id][$i]['attach_id']),
			...
			if ($link)
			{
				...
				//
				$template->assign_block_vars('postrow.attach.attachrow', array(
					'U_DOWNLOAD_LINK' => append_sid($modifiedphp_root .'download&id=' . $attachments['_' . $post_id][$i]['attach_id']),
					'S_UPLOAD_IMAGE' => $upload_image,
