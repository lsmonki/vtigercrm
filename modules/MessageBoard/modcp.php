 ...
 *
 *   $Id: modcp.php,v 1.9 2005/01/04 15:04:12 saraj Exp $
 *
 ***************************************************************************/
...
else
{
	...
	else if ($post_public)      // added by isaac
	{
		$mode = 'post_public';
	}
	else
...
//if (( !$is_auth['auth_mod'])    commented by isaac and below line added to allow 'post as public' action to move posts
if (( !$is_auth['auth_mod']) && ((intval($is_auth['auth_read']) != AUTH_SELF)))
{
...
switch( $mode )
{
	case 'delete':
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				'S_CONFIRM_ACTION' => append_sid($modifiedphp_root ."modcp"),
	...
	case 'move':
		...
		if ( $confirm )
		{
			...
			$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . $modifiedphp_root . "viewforum&" . POST_FORUM_URL . "=$old_forum_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				'S_MODCP_ACTION' => append_sid($modifiedphp_root . "modcp"),
				'S_HIDDEN_FIELDS' => $hidden_fields)
			);
		...
		//Added by Isaac for moving when 'Post AS Public'
		case 'post_public':
		$page_title = $lang['Mod_CP'];
		include($phpbb_root_path . 'includes/page_header.'.$phpEx);
		...
		if ( empty($HTTP_POST_VARS['topic_id_list']) && empty($topic_id) )
			...
			$new_forum_id = ($HTTP_GET_VARS['new_forum']);
			$old_forum_id = $forum_id;
			...
			if ( $new_forum_id != $old_forum_id )
			{
				...
				$message = $lang['Topics_Moved_Public'];
                $post_email=stripslashes($HTTP_GET_VARS['post_email']);
				if ($post_email != "")
          		{
					$message = $message . $lang['Topics_Moved_Email'] . '<b>' . $HTTP_GET_VARS['post_email'] . '</b><br /><br />';
				}
				else
				{
...
//
	case 'mergetopic':
		...
		else if ( $confirm )
		{
			...
			if ( !empty($topic_id) )
			{
				$redirect_page = append_sid($modifiedphp_root . "viewtopic&" . POST_TOPIC_URL . "=$topic_id_to");
				$message .= sprintf($lang['Click_return_topic'], '<a href="' . $redirect_page . '">', '</a>');
			}
			else
			{
				$redirect_page = append_sid($modifiedphp_root . "modcp&" . POST_FORUM_URL . "=$forum_id");
				$message .= sprintf($lang['Click_return_modcp'], '<a href="' . $redirect_page . '">', '</a>');
			}

			$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . append_sid($modifiedphp_root . "viewforum&" . POST_FORUM_URL . "=$old_forum_id") . '">', '</a>');
	...
	case 'mergepost':
		...
		if ( $confirm )
		{
			...
			$message = $lang['Posts_Merged'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid($modifiedphp_root . "viewtopic&" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
			message_die(GENERAL_MESSAGE, $message);
		}
		...
		{	// Step 0 - select the post you want to merge
			...
			if( ( $total_posts = $db->sql_numrows($result) ) > 0 )
			{
				...
				$template->assign_vars(array(
					...
					'S_MERGE_ACTION' => append_sid($modifiedphp_root ."modcp"),
...
//
	...
	case 'lock':
		...
		$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . $modifiedphp_root . "viewforum&" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
	...
	case 'split':
		...
		if ($post_id_sql != '')
		{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				$message = $lang['Topic_split'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . $modifiedphp_root . "viewtopic&" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
				message_die(GENERAL_MESSAGE, $message);
			}
		}
		else
		{
			...
			if( ( $total_posts = $db->sql_numrows($result) ) > 0 )
			{
				...
				$template->assign_vars(array(
					...
					'S_SPLIT_ACTION' => append_sid($modifiedphp_root ."modcp"),
					'S_HIDDEN_FIELDS' => $s_hidden_fields,
	...
	case 'ip':
		...
		$template->assign_vars(array(
			...
			'U_LOOKUP_IP' => $modifiedphp_root ."modcp&mode=ip&amp;" . POST_POST_URL . "=$post_id&amp;" . POST_TOPIC_URL . "=$topic_id&amp;rdns=$ip_this_post&amp;sid=" . $userdata['session_id'])
		);
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$template->assign_block_vars('userrow', array(
					...
					'U_PROFILE' => ($id == ANONYMOUS) ? "modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=" . $post_id . "&amp;" . POST_TOPIC_URL . "=" . $topic_id . "&amp;sid=" . $userdata['session_id'] : append_sid($modifiedphp_root . "profile&mode=viewprofile&amp;" . POST_USERS_URL . "=$id"),
					'U_SEARCHPOSTS' => append_sid($modifiedphp_root . "search&search_author=" . urlencode($username) . "&amp;showresults=topics"))
				);
