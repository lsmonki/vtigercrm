 ...
 *
 *   $Id: search.php,v 1.9 2005/01/04 15:04:12 saraj Exp $
 *
 *
 ***************************************************************************/
...
# Code added to search posts and topics from forum (or) blog separately - Godwin -->

if ( isset($HTTP_POST_VARS['search_categ']) || isset($HTTP_GET_VARS['search_categ']) )
{
        $search_categ = ( isset($HTTP_POST_VARS['search_categ']) ) ? $HTTP_POST_VARS['search_categ'] : $HTTP_GET_VARS['search_categ'];
}
else
{
        $search_categ = '';
...
// added by isaac for mini_cal MOD
...
//end isaac - mini_cal MOD
if ( !empty($HTTP_POST_VARS['search_time']) || !empty($HTTP_GET_VARS['search_time']))
{

				//	message_die(GENERAL_MESSAGE, '$Yes');
...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	//
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_id == 'startedtopics'|| $search_keywords != '' || $search_author != '' || $search_id == 'mini_cal' || $search_id == 'mini_cal_events' )
	{
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || ( $search_author != '' && $search_keywords == '' || $search_id == 'mini_cal' || $search_id == 'mini_cal_events')  )
		{
			if ( $search_id == 'newposts' )
			{
				if ( $userdata['session_logged_in'] )
				{
					$sql = "SELECT post_id
						...
						WHERE p.forum_id=f.forum_id AND f.cat_id <> 2 AND post_time >= " . (($search_time)? $search_time." AND post_time >= ":'' ).$userdata['user_lastvisit'];
				}
				else
				{
					redirect(append_sid($modifiedphp_root ."login&redirect=search&search_id=newposts", true));
				}
			...
			else if ( $search_id == 'egosearch' )
			{
				...
				{
					redirect(append_sid($modifiedphp_root ."login&redirect=search&search_id=egosearch", true));
				}
			...
			else
			{
				...
				$sql = "SELECT post_id
					FROM " . POSTS_TABLE . " p, " . FORUMS_TABLE . " f
						WHERE p.forum_id=f.forum_id  AND f.cat_id <> 2 and poster_id IN ($matching_userids)";
...
// MOD END - TOPICS I'VE STARTED - deejay
	...
	if ( $search_results != '' )
	{
		...
		# Code added to search posts and topics from forum (or) blog separately - Godwin -->
		if ( $search_categ == 'forum_categ' )
		{
			$categ_query = 'and f.cat_id <> 2';
		}
		elseif ( $search_categ == 'blog_categ' )
		{
			$categ_query = 'and f.cat_id = 2 and t.topic_auth <> 3 and u.weblog_visible <> 0';
		}
		# Godwin <--
		...
		{
			//Begin - To fix broken Blog links that is displayed when using the phpBB search feature - Isaac
			$blog_forum_id=$searchset[$i]['forum_id'];

			$sql_blog_link = "SELECT cat_allow_weblogs
				FROM " . FORUMS_TABLE . " f, " . CATEGORIES_TABLE . " c
				WHERE f.cat_id = c.cat_id AND f.forum_id=$blog_forum_id";

            if ( !($result = $db->sql_query($sql_blog_link)) )
			{
				message_die(GENERAL_MESSAGE, 'Could not obtain forum category ID ');
			}
			$catid_row = $db->sql_fetchrow($result);
			$blog_catid = $catid_row['cat_allow_weblogs'];
			if ($blog_catid == 1 )
			{
				$forum_url = append_sid($modifiedphp_root ."weblog&" . POST_FORUM_URL . '=' . $searchset[$i]['forum_id']);
				$topic_url = append_sid($modifiedphp_root ."weblog_entry&" . POST_ENTRY_URL . '=' . $searchset[$i]['topic_id'] . "&amp;highlight=$highlight_active");
				$post_url = append_sid($modifiedphp_root ."weblog_entry&" . POST_POST_URL . '=' . $searchset[$i]['post_id'] . "&amp;highlight=$highlight_active") . '#' . $searchset[$i]['post_id'];
			}
			else
			{
				$forum_url = append_sid($modifiedphp_root ."viewforum&" . POST_FORUM_URL . '=' . $searchset[$i]['forum_id']);
				$topic_url = append_sid($modifiedphp_root ."viewtopic&" . POST_TOPIC_URL . '=' . $searchset[$i]['topic_id'] . "&amp;highlight=$highlight_active");
				$post_url = append_sid($modifiedphp_root ."viewtopic&" . POST_POST_URL . '=' . $searchset[$i]['post_id'] . "&amp;highlight=$highlight_active") . '#' . $searchset[$i]['post_id'];
			}
			//End - Isaac - Broken blog links in search results
			...
			if ( $show_results == 'posts' )
			{
				...
				$poster = ( $searchset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . append_sid($modifiedphp_root ."profile&mode=viewprofile&amp;" . POST_USERS_URL . "=" . $searchset[$i]['user_id']) . '">' : '';
			...
			else
			{
				...
				if ( $searchset[$i]['topic_status'] == TOPIC_MOVED )
				{
					...
					$folder_image = '<img src="' . $phpbb_root_path . $images['folder'] . '" alt="' . $lang['No_new_posts'] . '" />';
					$newest_post_img = '';
				}
				else
				{
				...
				$topic_author = ( $searchset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . append_sid($modifiedphp_root ."profile&mode=viewprofile&amp;" . POST_USERS_URL . '=' . $searchset[$i]['user_id']) . '">' : '';
				...
				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? $searchset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid($modifiedphp_root ."profile&mode=viewprofile&amp;" . POST_USERS_URL . '='  . $searchset[$i]['id2']) . '">' . $searchset[$i]['user2'] . '</a>';

				//Added by Isaac - to fix the broken blog links when search result is displayed
				if ($blog_catid == 1 )
				{
					$last_post_url = '<a href="' . append_sid($modifiedphp_root ."weblog_entry&"  . POST_POST_URL . '=' . $searchset[$i]['topic_last_post_id']) . '#' . $searchset[$i]['topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" border="0" /></a>';
				}
				else
				{
					$last_post_url = '<a href="' . append_sid($modifiedphp_root ."viewtopic&"  . POST_POST_URL . '=' . $searchset[$i]['topic_last_post_id']) . '#' . $searchset[$i]['topic_last_post_id'] . '"><img src="' . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" border="0" /></a>';
				}
				//End - Isaac
		...
		$base_url = "search&search_id=$search_id";
...
// godwin - extra time added
$previous_days = array(0,15,30,45,60,120,180,360, 720, 1440, 2880, 4320, 10080, 20160, 43200, 129600, 259200, 524160);
$previous_days_text = array($lang['All_Posts'],
$lang['15_min'],$lang['30_min'],$lang['45_min'],$lang['1_Hour'],$lang['2_Hour'],$lang['3_Hour'],$lang['6_Hour'],$lang['12_Hour'],
$lang['1_Day'], $lang['2_Days'],$lang['3_Days'],$lang['7_Days'], $lang['2_Weeks'], $lang['1_Month'], $lang['3_Months'], $lang['6_Months'], $lang['1_Year']);
...
$template->assign_vars(array(
	...
	'L_FORUM' => $lang['Forum'],
	'L_TOPICS' => $lang['Topics'],
	'L_BLOGS' => $lang['Weblog_name_plural'],		// Godwin
	'L_POSTS' => $lang['Posts'],

	'S_SEARCH_ACTION' => append_sid($modifiedphp_root ."search&mode=results"),
