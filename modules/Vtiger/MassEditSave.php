<?php
/*+********************************************************************************
 * The contents of this file are subject to the vtiger CRM Public License Version 1.0
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is:  vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
 *********************************************************************************/
global $currentModule, $rstart;

$focus = CRMEntity::getInstance($currentModule);

$commentUpdateModules = array('HelpDesk', 'Faq');

$viewid = vtlib_purify($_REQUEST['viewname']);
$idstring = vtlib_purify($_REQUEST['massedit_recordids']);
$excludedRecords = vtlib_purify($_REQUEST['excludedRecords']);
$return_module = vtlib_purify($_REQUEST['massedit_module']);
$searchurl = vtlib_purify($_REQUEST['searchurl']);
$input = $_REQUEST;
if (!empty ($searchurl)) {
	$searchurl = explode('&', $searchurl);
	foreach ($searchurl as $key => $value) {
		$value = explode('=', $value);
		if (count($value) == 2) {
			$input[$value[0]] = $value[1];
		}
	}
}

$idlist = getSelectedRecords($input, $currentModule, $idstring, $excludedRecords);
$idlist = implode(';',$idlist);
$return_action = 'index';

//Added to fix 4600
$url = getBasic_Advance_SearchURL();
if(isset($_REQUEST['start']) && $_REQUEST['start']!='') {
	$rstart = "&start=".vtlib_purify($_REQUEST['start']);
}

if(isset($idlist) && $masseditCheck) {
	$recordids = explode(';', $idlist);
	for($index = 0; $index < count($recordids); ++$index) {
		$recordid = $recordids[$index];
		if($recordid == '') continue;
		if(isPermitted($currentModule,'EditView',$recordid) == 'yes') {
			// Reset column_fields of module instance for re-use.
			foreach($focus->column_fields as $key=>$value) {
			    $focus->column_fields[$key] = '';
			}
			// Save each module record with update value.
			$focus->retrieve_entity_info($recordid, $currentModule);
			$focus->mode = 'edit';
			$focus->id = $recordid;
			foreach($focus->column_fields as $fieldname => $val) {
				if(isset($_REQUEST[$fieldname."_mass_edit_check"])) {
					if($fieldname == 'assigned_user_id') {
						if($_REQUEST['assigntype'] == 'U') {
							$value = $_REQUEST['assigned_user_id'];
						} elseif($_REQUEST['assigntype'] == 'T') {
							$value = $_REQUEST['assigned_group_id'];
						}
					} else {
						if(is_array($_REQUEST[$fieldname])) {
							$value = $_REQUEST[$fieldname];
						} else {
							$value = trim($_REQUEST[$fieldname]);
						}
					}
					$focus->column_fields[$fieldname] = $value;
				} else {
					$focus->column_fields[$fieldname] = decode_html($focus->column_fields[$fieldname]);
				}
			}
			if ($avoidCommentUpdate) {
				// Avoiding comment update on ticket
				$focus->column_fields['comments'] = '';
				$_REQUEST['comments'] = '';
				// END
			}
			$focus->save($currentModule);
		}
	}
}

$parenttab = getParentTab();
header("Location: index.php?module=$return_module&action=$return_action&parenttab=$parenttab$rstart");
?>