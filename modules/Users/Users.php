...
class Users {
	...
	var $tab_name = Array('vtiger_users','vtiger_attachments','vtiger_user2role','vtiger_user2org','vtiger_user2orgunit');	
	...
	function saveentity($module)
	{
		...
		foreach($this->tab_name as $table_name)
		{
			...
			elseif($table_name == 'vtiger_user2org')
			{
				$this->insertIntoOrgRel($this->id,$module);
			}
			elseif($table_name == 'vtiger_user2orgunit')
			{
				$this->insertIntoOrgUnitRel($this->id,$module);
			}
			else
			{
	...
	/** Function to insert values into the organization relation table
  	  * @param $id -- entity id:: Type integer
  	  * @param $module -- module:: Type varchar
 	 */
	function insertIntoOrgRel($id,$module)
	{
	    global $log;
	    $log->debug("Entering into insertIntoOrgRel($id,$module) method.");

	    //Prepare an array of items to add to and to delete from the database
	    $assign = array();
	    if( is_array( $this->column_fields["assigned_org"])) {
		foreach( $this->column_fields["assigned_org"] as $key=>$multivalue) {
		    $assign[] = $multivalue;
		}
	    }
	    $delete = array();
	    ...
	    //Checking whether an entry is already is present in the vtiger_table to update
	    $insertion_mode = $this->mode;
	    if($insertion_mode == 'edit') {
		$sql = "select organizationname,primarytag from vtiger_user2org where userid=".$id;
		$result = $this->db->query($sql);
		if($this->db->num_rows($result) >= 1) {

		    while($result_set = $this->db->fetch_array($result)) {
			$current = $result_set['organizationname'];
			if( $result_set['primarytag'] == 1) 
		...
		$sql = "delete from vtiger_user2org where userid=".$id;
		$result = $this->db->query($sql);
	    }

	    //Add/Delete item from the database according to the arrays just
	    //set up
	    foreach( $assign as $org) {
		$sql = "insert into vtiger_user2org (userid,organizationname) values (".$id.",'".$org."')";
		$result = $this->db->query($sql);
	    }
	    foreach( $delete as $org) {
		$sql = "delete from vtiger_user2org where userid=".$id." and organizationname='".$org."'";
		$result = $this->db->query($sql);
	    }

	    //Update the primary organization flag
	    if( $current_primary != '' && $current_primary != $this->column_fields["primary_org"]) {
		$sql = "update vtiger_user2org set primarytag=0 
			    where userid=".$id." and organizationname='".$current_primary."'";
		$result = $this->db->query($sql);
	    }
	    if( $this->column_fields["primary_org"] != '') {
		$sql = "update vtiger_user2org set primarytag=1
			    where userid=".$id." and organizationname='".$this->column_fields["primary_org"]."'";
		$result = $this->db->query($sql);
	    }

	    //Done
	    $log->debug("Exiting from insertIntoOrgRel($id,$module) method.");
	}


	/** Function to insert values into the organization unit relation table
  	  * @param $id -- entity id:: Type integer
  	  * @param $module -- module:: Type varchar
 	 */
	function insertIntoOrgUnitRel($id,$module)
	{
	    global $log;
	    $log->debug("Entering into insertIntoOrgUnitRel($id,$module) method.");

	    //Prepare an array of items to add to and to delete from the database
	    $assign = array();
	    if( is_array( $this->column_fields["primary_orgunits"])) {
		foreach( $this->column_fields["primary_orgunits"] as $key=>$multivalue) {
		    $organdunit = split( ' - ', $multivalue);
	    ...
	    $insertion_mode = $this->mode;
	    if($insertion_mode == 'edit') {
		$sql = "select orgunitid from vtiger_user2orgunit where userid=".$id." and deleted=0";
		$result = $this->db->query($sql);
		if($this->db->num_rows($result) >= 1) {
		...
		$sql = "delete from vtiger_user2orgunit where userid=".$id;
		$result = $this->db->query($sql);
	    }

	    //Add/Delete item from the database according to the arrays just
	    //set up
	    if( is_array( $assignid)) {
		foreach( $assignid as $orgunitid) {
		    $sql = "insert into vtiger_user2orgunit (userid,orgunitid,primarytag) values (".$id.",'".$orgunitid."',1)";
		    $result = $this->db->query($sql);
		}
	    }
	    if( is_array( $delete)) {
		foreach( $delete as $orgunitid) {
		    $sql = "delete from vtiger_user2orgunit where userid=".$id." and orgunitid='".$orgunitid."'";
		    $result = $this->db->query($sql);
		}
	    }

	    //Done
	    $log->debug("Exiting from insertIntoOrgUnitRel($id,$module) method.");
	}
