...
echo ""
bin\apache -k stop -n vtigercrm502
bin\apache -k uninstall -n vtigercrm502
...
echo ""
echo ""
echo "Installing  vtigercrm502 apache service after sleeping for 10 seconds"
echo ""
%SLEEP_STR% -n 10 127.0.0.1>nul
bin\apache -k install -n vtigercrm502 -f conf\httpd.conf
echo ""
echo "Starting  vtigercrm502 apache service"
echo ""
bin\apache -n vtigercrm502 -k start
IF ERRORLEVEL 1 goto stopservice
goto checkmysql
...
:checkdatabase
echo ""
echo "check to see if vtigercrm502 database already exists"
echo ""
mysql --port=%mysql_port% --user=%mysql_username% --password=%mysql_password% -e "show databases like 'vtigercrm502'" | "%WINDIR%\system32\find.exe" "vtigercrm502" > NUL
IF ERRORLEVEL 1 goto dbnotexists
echo ""
ECHO  "vtigercrm502 database exists"
echo ""
goto end
...
:dbnotexists
echo ""
ECHO "vtigercrm502 database does not exist"
echo ""
echo %cd%
echo ""
echo "Proceeding to create database vtigercrm502 and populate the same"
echo ""
mysql --user=%mysql_username% --password=%mysql_password% --port=%mysql_port% -e "create database if not exists vtigercrm502"
echo ""
echo "vtigercrm502 database created"
echo ""
goto end
