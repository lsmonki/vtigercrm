...
echo ""
echo "Checking if the vtigercrm502 database already exists"
echo ""
echo "select 1" | ./bin/mysql --user=$mysql_username --password=$mysql_password  --port=$mysql_port --socket=$mysql_socket -D vtigercrm502 >/dev/null
if [ $? -ne 0 ]; then
	echo ""
	echo "Database vtigercrm502 does not exist. Creating database vtigercrm502"
	echo ""
	./bin/mysql --user=$mysql_username --password=$mysql_password  --port=$mysql_port --socket=$mysql_socket -e "create database if not exists vtigercrm502"
fi
