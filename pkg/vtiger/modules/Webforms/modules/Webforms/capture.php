		...
		try {
			...
			foreach ($webformFields as $webformField) {
				if($webformField->getDefaultValue()!=null){
					$parameters[$webformField->getFieldName()] = $webformField->getDefaultValue();
				}else{
					$parameters[$webformField->getFieldName()] = vtlib_purify($request[$webformField->getNeutralizedField()]);
				}
