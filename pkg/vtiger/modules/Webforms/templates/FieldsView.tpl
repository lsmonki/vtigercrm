	...
	<tr style="height:25px" id="field_row">
		...
		{else}
			{if $WEBFORMID}
				{if $WEBFORM->isWebformField($WEBFORMID,$field.name) eq true}
					<input type="checkbox" name="fields[]"  record="false" checked="checked" value="{$field.name}" onClick=Webforms.showHideElement('value[{$field.name}]','required[{$field.name}]','jscal_trigger_{$field.name}')>
				{else}
					<input type="checkbox" name="fields[]"   record="false" value="{$field.name}" onClick=Webforms.showHideElement('value[{$field.name}]','required[{$field.name}]','jscal_trigger_{$field.name}')>
				{/if}
			{else}
					<input type="checkbox" name="fields[]"  record="false" value="{$field.name}" onClick=Webforms.showHideElement('value[{$field.name}]','required[{$field.name}]','jscal_trigger_{$field.name}')>
			{/if}
		...
		{if $WEBFORMID && $WEBFORM->isWebformField($WEBFORMID,$field.name) eq true }
			...
			{elseif $field.type.name eq text}
					<textarea rows="2" onblur="this.className='detailedViewTextBox'" onfocus="this.className='detailedViewTextBoxOn'" class="detailedViewTextBox"  id="value[{$field.name}]" name="value[{$field.name}]"  value="{$WEBFORM->retrieveDefaultValue($WEBFORMID,$field.name)}"></textarea>
			...
			{else}
					...
					{else}
						<input type="text" onblur="this.className='detailedViewTextBox';sensex_info()" onfocus="this.className='detailedViewTextBoxOn';" class="detailedViewTextBox" id="value[{$field.name}]"  name="value[{$field.name}]" value="{$WEBFORM->retrieveDefaultValue($WEBFORMID,$field.name)}" style="display:inline;"></input>
					{/if}
			{/if}
		{else}
			{if $field.mandatory eq 1}
