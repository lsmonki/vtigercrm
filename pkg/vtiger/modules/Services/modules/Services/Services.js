    ...
    if(document.selectall.selected_id != undefined)
    {
		...
		if ( x == undefined) {
			if (document.selectall.selected_id.checked) {
				...
				var tmp = z[c];
				set_return_inventory(tmp[0],tmp[1],tmp[2],tmp[3],parseInt(tmp[4]),tmp[5]);
				y=1;
			} else {
			...
			for(i = 0; i < x ; i++) {
				if(document.selectall.selected_id[i].checked) {
					...
					var tmp = z[c];
					if(y>0) {
						row_id = window.opener.fnAddServiceRow(mod,image_pth);
					}				
					if(y==0) {
						set_return_inventory(tmp[0],tmp[1],tmp[2],tmp[3],parseInt(tmp[4]),tmp[5]);
					} else {
						set_return_inventory(tmp[0],tmp[1],tmp[2],tmp[3],parseInt(row_id),tmp[5]);
					}
					y=y+1;
				}
			}
...
function fnAddServiceRow(module,image_path){
	...
	coltwo.innerHTML= '<table border="0" cellpadding="1" cellspacing="0" width="100%"><tr><td class="small"><input id="productName'+count+'" name="productName'+count+'" class="small" style="width: 70%;" value="" readonly="readonly" type="text"><input id="hdnProductId'+count+'" name="hdnProductId'+count+'" value="" type="hidden">'+
						'<img id="searchIcon'+count+'" src="themes/images/search.gif" style="cursor: pointer;" onclick="servicePickList(this,\''+module+'\','+count+')" align="absmiddle">'+
