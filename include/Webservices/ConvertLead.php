...
function vtws_convertlead($leadId, $assignedTo, $accountName, $avoidPotential, $potential, $transferRelatedRecordsTo, $user) {
    ...
    if ($rowCount != 0 && vtlib_isModuleActive('Accounts') === true) {
        ...
        if (!empty($accountName)) {
            ...
            if ($accountHandler->getMeta()->hasWriteAccess()) {
                ...
                $mandatoryfields = $accountHandler->getMeta()->getMandatoryFields();
                ...
                if (preCreate('Accounts', $mandatoryfields, $account) == false) {
                    $log->fatal("account");
                    return null;
                }
    ...
    if ($contactHandler->getMeta()->hasWriteAccess()) {
        ...
        $mandatoryfields = $contactHandler->getMeta()->getMandatoryFields();
        foreach ($customFieldMapping as $leadFieldId => $mappingDetails) {
        ...
        if (preCreate('Contacts', $mandatoryfields, $contact) == false) {
            $log->fatal("contactt" . "accountid" . $accountId);
            return null;
        }
        $contact = vtws_create('Contacts', $contact, $user);

        if ($contact === null) {
            $contact = vtws_delete($accountId, $user);
