...
class VtigerCRMObjectMeta{
		...
		function retrieveMeta(){
			...
			foreach($cv->module_list[$this->objectName] as $key=>$value){
				$this->retrieveMetaForBlock($value);
			}
		...
		private function retrieveMetaForBlock($block){
			...
			for($i=0; $i<$noofrows; $i++){
				...
				if(in_array($uitype, $nonReferenceArray)){
					continue;
				}else if(in_array($uitype,array_keys($referenceArray))){
					...
					if($type == "reference"){
						...
						array_push($nonReferenceArray,$uitype);
					}
				} 
			}
