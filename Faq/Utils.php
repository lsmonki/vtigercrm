...
function getLatestlyCreatedFaqList()
{
	...
	$faq_array = $_SESSION['faq_array'];
	$list = '<div class="addimage">'.$mod_strings['LBL_RECENTLY_CREATED'].'</div>';
...
function ListFaqsPerCategory($category_index)
{
	...
	$list = '<div class="addimage">'.$mod_strings['LBL_CATEGORY'].': '.$category.'</div>';
	$list .= '<br><table width="100%" border="0" cellspacing="0" cellpadding="0" class="dummy">';
...
function ListFaqsPerProduct($productid)
{
	...
	$product_array = $_SESSION['product_array'];
	$faq_array = $_SESSION['faq_array'];
	$list = '<div class="addimage">'.$mod_strings['LBL_PRODUCT'].': '.getProductname($productid).'</div>';
	$list .= '<br><table width="100%" border="0" cellspacing="0" cellpadding="0" class="dummy">';
...
function getArticleIdTime($faqid,$product_id,$faqcategory,$faqcreatedtime,$faqmodifiedtime)
{
	...
	$list .='<div id="faqDetail" onMouseOver="fnShowDiv(\'faqDetail\')" onMouseOut="fnHideDiv(\'faqDetail\')">
		 <table class="fagView" cellpadding="0" cellspacing="0">
		   <tr>
			<td align="right"><b>'.$mod_strings['LBL_ARTICLE_ID'].': </b></td><td align="left"><b>'.$faqid.'</b></td>
		   </tr>
		   <tr>
			<td align="right">'.$mod_strings['LBL_PRODUCT'].': </td><td align="left">'.getProductName($product_id).'</td>
		   </tr>
		   <tr>
			<td align="right">'.$mod_strings['LBL_CATEGORY'].': </td><td align="left">'.$faqcategory.'</td>
		   </tr>
		   <tr>
			<td align="right">'.$mod_strings['LBL_CREATED_DATE'].': </td><td align="left">'.substr($faqcreatedtime,0,10).'</td>
		   </tr>
		   <tr>
			<td align="right" nowrap>'.$mod_strings['LBL_MODIFIED_DATE'].': </td><td align="left">'.substr($faqmodifiedtime,0,10).'</td>
		   </tr>
		</table>
		</div>';
...
function getPageOption()
{
	...
	$list .= '
			<table width="100%" border="0" cellspacing="3" cellpadding="3">
		   	   <tr>
				<td width="18" align="center"><img src="images/print.gif" valign="absmiddle"></td><td><a href="javascript:printPage()">'.$mod_strings['LBL_PRINT_THIS_PAGE'].'</a></td>
				<td width="18" align="center"><img src="images/email.gif" valign="absmiddle"></td><td><a href="javascript:sendAsEmail();">'.$mod_strings['LBL_EMAIL_THIS_PAGE'].'</a></td>
				<td width="18" align="center"><img src="images/favorite.gif" valign="absmiddle"></td><td><a href="javascript:addToFavorite();">'.$mod_strings['LBL_ADD_TO_FAVORITES'].'</a></td>
			   </tr>
			</table>
		';
...
function getSearchResult($search_text,$search_value,$search_by)
{
	...
	$list = '<div class="addimage">'.$mod_strings['LBL_SEARCH_RESULT'].'</div>';
	...
	if($search_value == 'All')
        {
			...
			if($search_text != '')
				...
				$record_exist = true;
					   ...
					   <tr>
						<td height="18" class="kbFAQInfo">'.$mod_strings['LBL_CATEGORY'].': '.$faq_array[$i]['category'].'</td>
					   </tr>
					   <tr>
						<td height="15"></td>
					   </tr>';
