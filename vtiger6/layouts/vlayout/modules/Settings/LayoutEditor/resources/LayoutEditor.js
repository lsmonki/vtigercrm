...
jQuery.Class('Settings_LayoutEditor_Js', {
	...
	createUpdatedBlockFieldsList : function() {
		...
		for(var index in  thisInstance.updatedBlocksList) {
			...
			var editFields = updatedBlock.find('.editFields').not('.emptyField');
	...
	showNewCustomField : function(result) {
		...
		fieldCopy.addClass('opacity editFields');
			...
			function(data) {
				...
				if(selectedvalue != null ){
					if(typeof(selectElemet.attr('multiple')) == 'undefined'){
						selectElemet.find('[value="'+selectedvalue+'"]').attr('selected','selected');
					} else {
						for (var i = 0; i < selectedvalue.length; i++) {
							selectElemet.find('[value="'+selectedvalue[i]+'"]').attr('selected','selected');
						}
					}
				}
