...
{strip}
	...
	{/foreach}
	<input type="hidden" class="allSourceValues" value='{ZEND_JSON::encode($SOURCE_PICKLIST_VALUES)}' />
			...
			<table class="table-bordered table-condensed themeTableColor pickListDependencyTable">
				<thead><tr class="blockHeader">
					{foreach item=SOURCE_PICKLIST_VALUE from=$SOURCE_PICKLIST_VALUES}
						<th class="{$SOURCE_FIELD}{$SOURCE_PICKLIST_VALUE}" style="border: none;
							{if !empty($MAPPED_VALUES) && !in_array($SOURCE_PICKLIST_VALUE, $MAPPED_SOURCE_PICKLIST_VALUES)}display: none;{/if}">
				...
				{foreach key=TARGET_INDEX item=TARGET_VALUE from=$TARGET_PICKLIST_VALUES name=targetValuesLoop}
					<tr>
					{foreach item=SOURCE_PICKLIST_VALUE from=$SOURCE_PICKLIST_VALUES}
						{assign var=targetValues value=$MAPPED_TARGET_PICKLIST_VALUES[$SOURCE_PICKLIST_VALUE]}
	...
	<div class="modal sourcePicklistValuesModal modalCloneCopy hide">
		...
		<div class="modal-body contentsBackground">
					...
					<td>
						<div class="control-group">
							<div class="controls row-fluid">
								<label class="checkbox"><input type="checkbox" class="sourceValue {$SOURCE_VALUE}"
									   value="{$SOURCE_VALUE}" 
									{if empty($MAPPED_VALUES) || in_array($SOURCE_VALUE, $MAPPED_SOURCE_PICKLIST_VALUES)} checked {/if}/>
