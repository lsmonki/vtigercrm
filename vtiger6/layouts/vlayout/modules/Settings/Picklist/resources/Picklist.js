...
var Settings_Picklist_Js = {
	...
	registerModuleChangeEvent : function() {
		jQuery('#pickListModules').on('change',function(e){
            ...
            if(selectedModule.length <= 0) {
                Settings_Vtiger_Index_Js.showMessage({'type': 'error','text':'JS_PLEASE_SELECT_MODULE'});
                return;
            }
			var params = {
	...
	duplicateItemNameCheck : function(container) {
		...
		jQuery.each(pickListValues,function(i,e){
			pickListValuesArr.push(e.toLowerCase());
		});
		...
		container.find('[name="addItemForm"]').on('submit',function(e){
			...
			if(validationResult == true) {
				...
				AppConnector.request(params).then(function(data) {
					...
					jQuery('#pickListValuesTable').find('tbody').append('<tr class="pickListValue cursorPointer" data-key="'+newValue+'"><td><img class="alignMiddle" src="'+dragImagePath+'" />&nbsp;&nbsp;'+newValue+'</td></tr>');
					app.hideModalWindow();
					var params = {
						title : app.vtranslate('JS_MESSAGE'),
			...
			if(validationResult == true) {
				...
				AppConnector.request(params).then(function(data) {
					if(typeof data.result != 'undefined'){
						app.hideModalWindow();
						var dragImagePath = jQuery('#dragImagePath').val();
						jQuery('[data-key="'+oldValue+'"]').replaceWith('<tr class="pickListValue cursorPointer" data-key="'+newValue+'"><td><img class="alignMiddle" src="'+dragImagePath+'" />&nbsp;&nbsp;'+newValue+'</td></tr>')
						var params = {
						title : app.vtranslate('JS_MESSAGE'),
