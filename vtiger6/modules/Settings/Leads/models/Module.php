...
class Settings_Leads_Module_Model extends Vtiger_Module_Model {
	...
	public function getMappingSupportedFieldIdsList() {
		if (!$this->supportedFieldIdsList) {
			...
			$presense = array(0, 2);
			$restrictedFieldIds = Settings_Leads_Mapping_Model::getRestrictedFieldIdsList();
			...
			if ($restrictedFieldIds) {
				$query .= 'AND fieldid NOT IN('. generateQuestionMarks($restrictedFieldIds). ')';
				$params = array_merge($params, $restrictedFieldIds);
			}
