 ...
 ********************************************************************************/ *}
				...
				<table class="listTable" border="0" cellpadding="3" cellspacing="0" width="100%">
					...
					{if $entries.blockid ne $RELPRODUCTSECTIONID && $entries.blocklabel neq ''}
						<tr><td colspan="5"><table width="100%" border="0" cellpadding="0" cellspacing="0"><tr><td  class="colHeader small" width="70%">{$entries.blocklabel}&nbsp;&nbsp;
						  </td>
						<td class="colHeader small"  align='right'> {if $smarty.foreach.outer.first}
						...
						{/if}<select name="display_status" align='right' style="border:1px solid #666666;font-family:Arial, Helvetica, sans-serif;font-size:11px; width:auto" onChange="changeShowstatus(this,'{$entries.tabid}','{$entries.blockid}',this.value)" id='display_status'>
	                		    <option value="show" {if $entries.display_status==1}selected{/if}>{$MOD.LBL_Show}</option>
								<option value="hide" {if $entries.display_status==0}selected{/if}>{$MOD.LBL_Hide}</option>			                </select></td></tr></table>
						</td>
						</tr>
						<tr>
						{if $entries.blockid ne $COMMENTSECTIONID}<!-- $entries.blockid ne $CUSTOMSECTIONID && -->
							{if $entries.no==0}
						...
						{foreach name=inner item=value from=$entries.field}
						 ...
						 {if $smarty.foreach.inner.first}
							 ...
							 {/if}
							 <!--&nbsp;&nbsp;<img src="{$IMAGE_PATH}editfield.gif" border="0" style="cursor:pointer;" onclick="fnvshobj(this,'createcf');getCreateCustomFieldForm('{$MODULE}','{$value.blockid}','{$value.tabid}','{$value.uitype}','{$entries.blocklabel}','{$value.fieldselect}','edit')" alt="Edit" title="Edit">
							 {if $entries.blocklabel=='Custom Information'}
							 &nbsp;&nbsp;<img style="cursor:pointer;" onClick="deleteCustomField('{$value.fieldselect}','{$MODULE}', '{$value.columnname}', '{$value.uitype}')" src="{$IMAGE_PATH}delete.gif" border="0"  alt="Delete" title="Delete"/>
							 {/if}-->
						...
						{elseif $smarty.foreach.inner.last}
							...
							{/if}
							 ...
							 <td width="19%" align = right><!--<img src="{$IMAGE_PATH}editfield.gif" border="0" style="cursor:pointer;" onclick="fnvshobj(this,'createcf');getCreateCustomFieldForm('{$MODULE}','{$value.blockid}','{$value.tabid}','{$value.uitype}','{$entries.blocklabel}','{$value.fieldselect}','edit')" alt="Edit" title="Edit">
							 {if $entries.blocklabel=='Custom Information'}
							&nbsp;&nbsp;<img style="cursor:pointer;" onClick="deleteCustomField('{$value.fieldselect}','{$MODULE}', '{$value.columnname}', '{$value.uitype}')" src="{$IMAGE_PATH}delete.gif" border="0"  alt="Delete" title="Delete"/>
							{/if}-->
							&nbsp;&nbsp;<img src="{$IMAGE_PATH}plus_layout.gif" border="0" style="cursor:pointer;" onclick="fnvshobj(this,'createcf');getCreateCustomFieldForm('{$MODULE}','{$value.blockid}','{$value.tabid}','','{$entries.blocklabel}','{$value.fieldselect}','')" alt="Add" title="Add">
							
							</td>{if $value.no % 2 == 0}<td style="border-left:solid 1px #ccc; width:1px;">
						...
						{else}		
							...
							</td>{if $value.no % 2 == 0}<td style="border-left:solid 1px #ccc; width:1px;"></td>	{/if}
