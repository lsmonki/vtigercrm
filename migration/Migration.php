...
class Migration
{
	...
	function takeDatabaseDump($host_name,$mysql_port,$mysql_username,$mysql_password,$dbname)
	{
		...
		echo '<br> 4.2 Database Dump has been taken and the file is ==> '.$dump_filename;
	...
	function dropDatabase($conn,$dbname)
	{
		...
		echo '<br> 4.5(Alpha) Database dropped.';
	}
	function createDatabase($conn,$dbname)
	{
		...
		echo '<br> 4.5(Alpha) Database created.';
	}
	...
	function applyDumpData($host_name,$mysql_port,$mysql_username,$mysql_password,$dbname,$dumpfile)
	{
		if($mysql_password != '')
		{
			exec("mysql -u".$mysql_username." -p".$mysql_password." -h ".$host_name." --port=".$mysql_port." ".$dbname." < ".$dumpfile);
		}
		else
		{
			exec("mysql -u".$mysql_username." -h ".$host_name." --port=".$mysql_port." ".$dbname." < ".$dumpfile);
		}

		echo '<br> 4.2 Database Dump has been applied to 4.5(Alpha) Database.';
	}
	...
	function modifyDatabase42P2_to_45Alpha($conn)
	{
		...
		$conn->println("Database Modifications for 4.2 Patch2 ==> 4.5(Alpha) Dev 3 Starts here.");
		echo "<br><br><b>Database Modifications for 4.2 Patch2 ==> 4.5(Alpha) Dev 3 Starts here.....</b><br>";
		$conn->println("Mickie ---- Starts");

		/****************** 4.5(Alpha) dev version 1 Database changes -- Starts*********************/
		...
		/****************** 4.5(Alpha) dev version 1 Database changes -- Ends*********************/
		...
		/****************** 4.5(Alpha) dev version 2 Database changes -- Starts*********************/
		...
		/****************** 4.5(Alpha) dev version 2 Database changes -- Ends*********************/
		...
		/****************** 4.5(Alpha) dev version 3 Database changes -- Starts*********************/
		...
		/****************** 4.5(Alpha) dev version 3 Database changes -- Ends*********************/
		...
		$conn->println("Database Modifications for 4.2 Patch2 ==> 4.5(Alpha) Dev 3 ends here.");
		echo "<br><br><b>Database Modifications for 4.2 Patch2 ==> 4.5(Alpha) Ends here.....</b><br>";
	}
	...
	function migrate($same_databases)
	{
		...
		global $conn;
		$this->conn->println("Database Migration from 4.2 Patch2 to 4.5(Alpha) Starts.");
		...
		{
			echo '<br> Databases are different. So drop the current 4.5(Alpha) database and create. Also apply the dump of 4.2';
		...
		$this->modifyDatabase42P2_to_45Alpha($conn);
		
		$this->conn->println("Database Modifications Ends......");
		$this->conn->println("Database Migration from 4.2 Patch2 to 4.5(Alpha) Finished.");
	}
