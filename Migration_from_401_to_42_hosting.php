...
include("migrate_functions.php");

global $conn;

$db_host_name = ''; //This is the host name only eg. my-server
$mysql_port = ''; // This is the port where your mysql server is running
$mysql_username = '';//This is the user name to access mysql
$mysql_password = '';//This is the password to access mysql
$mysql_401_dbname = 'vtigercrm4_0_1'; //This is vtiger 4_0_1 dbname. Give the exact db name.
$mysql_42_dbname = 'vtigercrm4_2';  //This is vtiger 4_2 GA dbname. Give the exact db name.
...
if($conn)
{
	if(!$db1_status)
	{
		echo 'Please check whether <b>'.$mysql_401_dbname.'</b> database is exist or not';
	}
	if(!$db2_status)
	{
		echo 'Please check whether <b>'.$mysql_42_dbname.'</b> database is exist or not';
	}
	if($db1_status && $db2_status)
	{
		echo '<br>Mysql server is running and the databases are found.<br><br>';
		echo 'Going to start the Migration from <b>'.$mysql_401_dbname.'</b> to <b>'.$mysql_42_dbname.' GA</b>';

		makeDatabaseDumpProcess($conn,$db_host_name,$mysql_port,$mysql_username,$mysql_password,$mysql_401_dbname,$mysql_42_dbname);
	}
...
function makeDatabaseDumpProcess($conn,$hostname,$port,$username,$password,$db_401,$db_42)
{
        //Take the dump of 4_0_1 to modify to 4_2
        echo '<br><br> Going to take the  Dump of vtigercrm4_0_1...<br>';
        exec("echo 'set FOREIGN_KEY_CHECKS=0;' > dump_vtigercrm4_0_1.txt");
	exec("mysqldump -u ".$username." --password=".$password." --port=".$port." -h ".$hostname." ".$db_401." >> dump_vtigercrm4_0_1.txt");
	exec("echo 'set FOREIGN_KEY_CHECKS=1;' >> dump_vtigercrm4_0_1.txt");
        echo '<br><br> Database dump for '.$db_401.' is taken successfully.';

        //Create new database to modify without affecting 4_0_1
        mysql_query("create database vtigercrm4_0_1_dump");
        echo '<br><br> Created New Database for dump values.';

        //Put the dumped value which is taken from 4_0_1 to new dumped database
	exec("mysql -u ".$username." --password=".$password." --port=".$port." -h ".$hostname." vtigercrm4_0_1_dump < dump_vtigercrm4_0_1.txt");
        echo '<br><br> Data are put into the vtigercrm4_0_1_dump Database.';

        //To Modify the dumped database of 4_0_1 to suite with 4_2
        makechanges($hostname,$port,$username,$password);

        //Take dump from the changed database which is suitable to vtigercrm4_2
        exec("echo 'set FOREIGN_KEY_CHECKS=0;' > dump_to_vtigercrm4_2.txt");
	exec("mysqldump -u ".$username." --password=".$password." --port=".$port." -h ".$hostname." vtigercrm4_0_1_dump >> dump_to_vtigercrm4_2.txt");
	exec("echo 'set FOREIGN_KEY_CHECKS=1;' >> dump_to_vtigercrm4_2.txt");

        //Drop and Create New database 4_2
        mysql_query("drop database vtigercrm4_2");
        mysql_query("create database vtigercrm4_2");

        //Put the dumped value which is taken from modified dump_4_0_1 to new 4_2 database
	exec("mysql -u ".$username." --password=".$password." --port=".$port." -h ".$hostname." vtigercrm4_2 < dump_to_vtigercrm4_2.txt");
        echo '<br><br> Data are put into the 4_2 Database which is run in the port :'.$port;

}

function GETUNIQUEID($conn,$tablename)
{
        $db_status = mysql_select_db('vtigercrm4_0_1_dump',$conn);// or die("db can't selected");
        $res = mysql_query("select max(fieldid) count from field");// or die("can't queried");
        $id = mysql_result($res,0,'count');// or die("can't get row");
	$id++;
...
function GETTABID($module)
{
	...
	function makechanges($hostname,$port,$username,$password)
	{
		...
		echo '<br> '.$sql102 .' <br> ';
		$res102 = mysql_query($sql102);

		$sql103="INSERT INTO tab VALUES (21,'Orders',0,18,'Orders','','',1)";
		echo '<br> '.$sql103 .' <br> ';
		$res103 = mysql_query($sql103);

		$sql104="INSERT INTO tab VALUES (22,'SalesOrder',2,19,'SalesOrder','','',1)";
		echo '<br> '.$sql104 .' <br> ';
		$res104 = mysql_query($sql104);

		$sql105="INSERT INTO tab VALUES (23,'Invoice',0,20,'Invoice','','',1)";
		echo '<br> '.$sql105 .' <br> ';
		$res105 = mysql_query($sql105);

		$sql106="INSERT INTO tab VALUES (24,'Rss',0,21,'Rss','','',1)";
		echo '<br> '.$sql106 .' <br> ';
		$res106 = mysql_query($sql106);

		$sql107="INSERT INTO tab VALUES (25,'Reports',0,22,'Reports','','',1)";
		echo '<br> '.$sql107 .' <br> ';
		$res107 = mysql_query($sql107);


		/*
		   - $this->db->query("insert into field values (4,".GETUNIQUEID($conn,"field").",'accountid','contactdetails',1,'50','account_id','Account Name',1,0,0,100,6,1,1,'I~M')");
		   + $this->db->query("insert into field values (4,".GETUNIQUEID($conn,"field").",'accountid','contactdetails',1,'51','account_id','Account Name',1,0,0,100,6,1,1,'I~O')");
		...
		echo '<br> '.$sql173 .' <br> ';
		$res173 = mysql_query($sql173);




		//Block - Begin Customer Portal
		$sql172="insert into field values (4,".GETUNIQUEID($conn,"field").",'portal','CustomerDetails',1,'56','portal','Portal User',1,0,0,100,1,4,1,'C~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (4,".GETUNIQUEID($conn,"field").",'support_start_date','CustomerDetails',1,'5','support_start_date','Support Start Date',1,0,0,100,2,4,1,'D~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (4,".GETUNIQUEID($conn,"field").",'support_end_date','CustomerDetails',1,'5','support_end_date','Support End Date',1,0,0,100,3,4,1,'D~O~OTH~GE~support_start_date~Support Start Date')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		/*
		   - $this->db->query("insert into field values (13,".GETUNIQUEID($conn,"field").",'contact_id','troubletickets',1,'57','contact_id','Contact Name',1,0,0,100,4,1,1,'I~O')");
		   + $this->db->query("insert into field values (13,".GETUNIQUEID($conn,"field").",'parent_id','troubletickets',1,'68','parent_id','Related To',1,0,0,100,4,1,1,'I~O')");
		 */
		...
		$sql172="insert into field values (13,".GETUNIQUEID($conn,"field").",'product_id','troubletickets',1,'59','product_id','Product Name',1,0,0,100,6,1,1,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (13,".GETUNIQUEID($conn,"field").",'severity','troubletickets',1,'15','ticketseverities','Severity',1,0,0,100,7,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
...
/*
		$sql172="insert into field values (13,".GETUNIQUEID($conn,"field").",'solution','troubletickets',1,'19','solution','Solution',1,0,0,100,1,4,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (13,".GETUNIQUEID($conn,"field").",'comments','ticketcomments',1,'19','comments','Add Comment',1,0,0,100,1,6,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
*/

		$sql176="insert into field values (14,".GETUNIQUEID($conn,"field").",'imagename','products',1,'69','imagename','Product Image',1,0,0,100,1,6,1,'V~O')"; 
		...
		$sql172="insert into field values (9,".GETUNIQUEID($conn,"field").",'reminder_time','activity_reminder',1,'30','reminder_time','Send Reminder',1,0,0,100,1,7,3,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (9,".GETUNIQUEID($conn,"field").",'recurringtype','recurringevents',1,'15','recurringtype','Recurrence',1,0,0,100,6,1,3,'O~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into field values (16,".GETUNIQUEID($conn,"field").",'reminder_time','activity_reminder',1,'30','reminder_time','Send Reminder',1,0,0,100,1,7,1,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into field values (15,".GETUNIQUEID($conn,"field").",'status','faq',1,'15','faqstatus','Status',1,0,0,100,3,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		echo '<br> '.$sql108 .' <br> ';
		$res108 = mysql_query($sql108);

		$profile1_id= mysql_result($res108,0,'profileid');

		$sql112="insert into profile2tab values (".$profile1_id.",18,0)";
		echo '<br> '.$sql112 .' <br> ';
		$res112 = mysql_query($sql112);
		...
		echo '<br> '.$sql109 .' <br> ';
		$res109 = mysql_query($sql109);
		$profile2_id= mysql_result($res109,0,'profileid');                        
		...
		echo '<br> '.$sql110 .' <br> ';
		$res110 = mysql_query($sql110);
		$profile3_id= mysql_result($res110,0,'profileid');                        
		...
		$sql172="insert into def_org_share values (".GETUNIQUEID($conn,'def_org_share').",18,2)";		
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into def_org_share values (".GETUNIQUEID($conn,'def_org_share').",19,2)";		
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into def_org_share values (".GETUNIQUEID($conn,'def_org_share').",20,2)";			
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into def_org_share values (".GETUNIQUEID($conn,'def_org_share').",21,2)";			
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into def_org_share values (".GETUNIQUEID($conn,'def_org_share').",22,2)";			
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into def_org_share values (".GETUNIQUEID($conn,'def_org_share').",23,2)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Potentials").",'get_opportunities',1,'Potentials',0)";
		echo '<br> >>>>>>>>>>>>>>>  '.$sql172 .' <br> ';
		mysql_query($sql172);	
		//mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Potentials").",'get_opportunities',1,'Potentials',0)");	

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Contacts").",'get_contacts',2,'Contacts',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Activities").",'get_activities',3,'Acivities',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("HelpDesk").",'get_tickets',4,'HelpDesk',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Activities").",'get_history',5,'History',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",0,'get_attachments',6,'Attachments',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Quotes").",'get_quotes',7,'Quotes',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Invoice").",'get_invoices',8,'Invoice',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("SalesOrder").",'get_salesorder',9,'Sales Order',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Products").",'get_products',10,'Products',0)");

		//Inserting Lead Related Lists	

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Leads").",".GETTABID("Activities").",'get_activities',1,'Activities',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Leads").",".GETTABID("Emails").",'get_emails',2,'Emails',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Leads").",".GETTABID("Activities").",'get_history',3,'History',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Leads").",0,'get_attachments',4,'Attachments',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Leads").",".GETTABID("Products").",'get_products',5,'Products',0)");

		//Inserting for contact related lists
		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Potentials").",'get_opportunities',1,'Potentials',0)");	

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Activities").",'get_activities',2,'Activities',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Emails").",'get_emails',3,'Emails',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("HelpDesk").",'get_tickets',4,'HelpDesk',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Quotes").",'get_quotes',5,'Quotes',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Orders").",'get_purchase_orders',6,'Purchase Order',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("SalesOrder").",'get_salesorder',7,'Sales Order',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Products").",'get_products',8,'Products',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Activities").",'get_history',9,'History',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",0,'get_attachments',10,'Attachments',0)");

		//Inserting Potential Related Lists	

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("Activities").",'get_activities',1,'Activities',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("Contacts").",'get_contacts',2,'Contacts',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("Products").",'get_products',3,'History',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",0,'get_stage_history',4,'Sales Stage History',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",0,'get_attachments',5,'Attachments',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("Quotes").",'get_Quotes',6,'Quotes',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("SalesOrder").",'get_salesorder',7,'Sales Order',0)");

		//Inserting Product Related Lists	

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("HelpDesk").",'get_tickets',1,'HelpDesk',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("Activities").",'get_activities',2,'Activities',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",0,'get_attachments',3,'Attachments',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("Quotes").",'get_quotes',4,'Quotes',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("Orders").",'get_purchase_orders',5,'Purchase Order',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("SalesOrder").",'get_salesorder',6,'Sales Order',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("Invoice").",'get_invoices',7,'Invoice',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("PriceBook").",'get_product_pricebooks',8,'PriceBook',0)");

		//Inserting Emails Related Lists	

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Emails").",".GETTABID("Contacts").",'get_contacts',1,'Contacts',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Emails").",0,'get_users',2,'Users',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Emails").",0,'get_attachments',3,'Attachments',0)");

		//Inserting HelpDesk Related Lists

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("HelpDesk").",".GETTABID("Activities").",'get_activities',1,'Activities',0)");


		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("HelpDesk").",0,'get_attachments',2,'Attachments',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("PriceBook").",14,'get_pricebook_products',2,'Products',0)");

		// Inserting Vendor Related Lists
		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Vendor").",14,'get_products',1,'Products',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Vendor").",21,'get_purchase_orders',2,'Products',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Vendor").",4,'get_contacts',3,'Contacts',0)");

		// Inserting Quotes Related Lists

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Quotes").",".GETTABID("Invoice").",'get_salesorder',1,'Sales Order',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Quotes").",9,'get_activities',2,'Activities',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Quotes").",9,'get_history',3,'History',0)");

		// Inserting Purchase order Related Lists

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Orders").",9,'get_activities',1,'Activities',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Orders").",0,'get_attachments',2,'Attachments',0)");

		// Inserting Sales order Related Lists

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("SalesOrder").",9,'get_activities',1,'Activities',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("SalesOrder").",0,'get_attachments',2,'Attachments',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("SalesOrder").",".GETTABID("Invoice").",'get_invoices',3,'Invoice',0)");

		// Inserting Invoice Related Lists

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Invoice").",9,'get_activities',1,'Activities',0)");

		mysql_query("insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Invoice").",0,'get_attachments',2,'Attachments',0)");
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Contacts").",'get_contacts',2,'Contacts',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Activities").",'get_activities',3,'Acivities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("HelpDesk").",'get_tickets',4,'HelpDesk',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",".GETTABID("Activities").",'get_history',5,'History',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",0,'get_attachments',6,'Attachments',0)";
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",20,'get_quotes',7,'Quotes',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",23,'get_invoices',8,'Invoice',0)";

		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Accounts").",22,'get_salesorder',9,'Sales Order',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Leads").",".GETTABID("Activities").",'get_activities',1,'Activities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Leads").",".GETTABID("Activities").",'get_history',3,'History',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Leads").",0,'get_attachments',4,'Attachments',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		//Inserting for contact related lists
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Potentials").",'get_opportunities',1,'Potentials',0)";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Activities").",'get_activities',2,'Activities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Emails").",'get_emails',3,'Emails',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("HelpDesk").",'get_tickets',4,'HelpDesk',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Activities").",'get_history',5,'History',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",0,'get_attachments',6,'Attachments',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Quotes").",'get_quotes',7,'Quotes',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Orders").",'get_purchase_orders',8,'Purchase Order',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("SalesOrder").",'get_salesorder',9,'Sales Order',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Contacts").",".GETTABID("Products").",'get_products',10,'Products',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("Activities").",'get_activities',1,'Activities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("Contacts").",'get_contacts',2,'Contacts',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("Products").",'get_products',3,'History',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",0,'get_stage_history',4,'Sales Stage History',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",0,'get_attachments',5,'Attachments',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("Quotes").",'get_Quotes',6,'Quotes',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Potentials").",".GETTABID("SalesOrder").",'get_salesorder',7,'Sales Order',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("HelpDesk").",'get_tickets',1,'HelpDesk',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("Activities").",'get_activities',2,'Activities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",0,'get_attachments',3,'Attachments',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Products").",".GETTABID("Quotes").",'get_quotes',4,'Quotes',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Emails").",".GETTABID("Contacts").",'get_contacts',1,'Contacts',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Emails").",0,'get_users',2,'Users',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Emails").",0,'get_attachments',3,'Attachments',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("HelpDesk").",".GETTABID("Activities").",'get_activities',1,'Activities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("HelpDesk").",0,'get_attachments',2,'Attachments',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("PriceBook").",14,'get_pricebook_products',2,'Products',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		// Inserting Vendor Related Lists
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Vendor").",14,'get_products',1,'Products',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Vendor").",21,'get_purchase_orders',2,'Products',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Vendor").",4,'get_contacts',3,'Contacts',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Quotes").",22,'get_salesorder',1,'Sales Order',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Quotes").",9,'get_activities',2,'Activities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Quotes").",9,'get_history',3,'History',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Orders").",9,'get_activities',1,'Activities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Orders").",0,'get_attachments',2,'Attachments',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("SalesOrder").",9,'get_activities',1,'Activities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("SalesOrder").",0,'get_attachments',2,'Attachments',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Invoice").",9,'get_activities',1,'Activities',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into relatedlists values(".GETUNIQUEID($conn,'relatedlists').",".GETTABID("Invoice").",0,'get_attachments',2,'Attachments',0)";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		*/  
		...
		$sql172="insert into inventorynotification(notificationid,notificationname,notificationsubject,notificationbody,label) values (".GETUNIQUEID($conn,"inventorynotification").",'InvoiceNotification','{PRODUCTNAME} Stock Level is Low','".$invoice_body." ','InvoiceNotificationDescription')";
		...
		$sql172="insert into inventorynotification(notificationid,notificationname,notificationsubject,notificationbody,label) values (".GETUNIQUEID($conn,"inventorynotification").",'QuoteNotification','Quote given for {PRODUCTNAME}','".$quote_body." ','QuoteNotificationDescription')";
		...
		$sql172="insert into inventorynotification(notificationid,notificationname,notificationsubject,notificationbody,label) values (".GETUNIQUEID($conn,"inventorynotification").",'SalesOrderNotification','Sales Order generated for {PRODUCTNAME}','".$so_body." ','SalesOrderNotificationDescription')";
		...
		$sql172="insert into field values (15,".GETUNIQUEID($conn,"field").",'product_id','faq',1,'59','product_id','Product Name',1,0,0,100,1,1,1,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'sales_start_date','products',1,'5','sales_start_date','Sales Start Date',1,0,0,100,5,1,1,'D~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'sales_end_date','products',1,'5','sales_end_date','Sales End Date',1,0,0,100,6,1,1,'D~O~OTH~GE~sales_start_date~Sales Start Date')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'usageunit','products',1,'15','usageunit','Usage Unit',1,0,0,100,1,3,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		/*
		   $sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'currency','products',1,'1','currency','Currency',1,0,0,100,3,2,1,'V~O')";
		   echo '<br> '.$sql172 .' <br> ';
		   $res172 = mysql_query($sql172);
		 */

		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'taxclass','products',1,'15','taxclass','Tax Class',1,0,0,100,4,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'website','products',1,'17','website','Website',1,0,0,100,12,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'glacct','products',1,'15','glacct','GL Account',1,0,0,100,18,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'handler','products',1,'52','assigned_user_id','Handler',1,0,0,100,5,3,1,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'vendor_id','products',1,'75','vendor_id','Vendor Name',1,0,0,100,13,1,1,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'qtyinstock','products',1,'1','qtyinstock','Qty In Stock',1,0,0,100,3,3,1,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'mfr_part_no','products',1,'1','mfr_part_no','Mfr PartNo',1,0,0,100,14,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'vendor_part_no','products',1,'1','vendor_part_no','Vendor PartNo',1,0,0,100,15,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'serialno','products',1,'1','serial_no','Serial No',1,0,0,100,16,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'productsheet','products',1,'1','productsheet','Product Sheet',1,0,0,100,17,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'reorderlevel','products',1,'1','reorderlevel','Reorder Level',1,0,0,100,4,3,1,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'qtyindemand','products',1,'1','qtyindemand','Qty In Demand',1,0,0,100,6,3,1,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block 4 Description Info
		//$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'product_description','products',1,'19','product_description','Description',1,0,0,100,1,4,1,'V~O')";
		//  echo '<br> '.$sql172 .' <br> ';
		//  $res172 = mysql_query($sql172);
		...
		$sql172="insert into field values (16,".GETUNIQUEID($conn,"field").",'due_date','activity',1,'5','due_date','End Date',1,0,0,100,5,1,1,'D~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (16,".GETUNIQUEID($conn,"field").",'recurringtype','recurringevents',1,'15','recurringtype','Recurrence',1,0,0,100,6,1,1,'O~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		//Block1
		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'vendorname','vendor',1,'2','vendorname','Vendor Name',1,0,0,100,1,1,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'phone','vendor',1,'1','phone','Phone',1,0,0,100,3,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'email','vendor',1,'13','email','Email',1,0,0,100,4,1,1,'E~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'website','vendor',1,'17','website','Website',1,0,0,100,5,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'glacct','vendor',1,'15','glacct','GL Account',1,0,0,100,6,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'category','vendor',1,'1','category','Category',1,0,0,100,7,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'createdtime','crmentity',1,'70','createdtime','Created Time',1,0,0,100,8,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'modifiedtime','crmentity',1,'70','modifiedtime','Modified Time',1,0,0,100,9,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		//Block 2
		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'street','vendor',1,'21','treet','Street',1,0,0,100,1,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'city','vendor',1,'1','city','City',1,0,0,100,2,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'state','vendor',1,'1','state','State',1,0,0,100,3,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'postalcode','vendor',1,'1','postalcode','Postal Code',1,0,0,100,4,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'country','vendor',1,'1','country','Country',1,0,0,100,5,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		//Block 3
		$sql172="insert into field values (18,".GETUNIQUEID($conn,"field").",'description','crmentity',1,'19','description','Description',1,0,0,100,1,3,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		//Block1
		$sql172="insert into field values (19,".GETUNIQUEID($conn,"field").",'bookname','pricebook',1,'2','bookname','Price Book Name',1,0,0,100,1,1,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (19,".GETUNIQUEID($conn,"field").",'active','pricebook',1,'56','active','Active',1,0,0,100,3,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (19,".GETUNIQUEID($conn,"field").",'createdtime','crmentity',1,'70','createdtime','Created Time',1,0,0,100,4,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (19,".GETUNIQUEID($conn,"field").",'modifiedtime','crmentity',1,'70','modifiedtime','Modified Time',1,0,0,100,5,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		//Block2
		$sql172="insert into field values (19,".GETUNIQUEID($conn,"field").",'description','crmentity',1,'19','description','Description',1,0,0,100,1,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		//Quote Details -- START
		//Block1
		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'subject','quotes',1,'2','subject','Subject',1,0,0,100,1,1,1,'V~M')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'potentialid','quotes',1,'76','potential_id','Potential Name',1,0,0,100,2,1,1,'I~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'quotestage','quotes',1,'15','quotestage','Quote Stage',1,0,0,100,3,1,1,'V~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'validtill','quotes',1,'5','validtill','Valid Till',1,0,0,100,4,1,1,'D~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'team','quotes',1,'1','team','Team',1,0,0,100,5,1,1,'V~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'contactid','quotes',1,'57','contact_id','Contact Name',1,0,0,100,6,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		//$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'currency','quotes',1,'1','currency','Currency',1,0,0,100,7,1,1,'V~O')";
		// echo '<br> '.$sql172 .' <br> ';
		//$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'carrier','quotes',1,'15','carrier','Carrier',1,0,0,100,8,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'subtotal','quotes',1,'1','hdnSubTotal','Sub Total',1,0,0,100,9,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'shipping','quotes',1,'1','shipping','Shipping',1,0,0,100,10,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'inventorymanager','quotes',1,'77','assigned_user_id1','Inventory Manager',1,0,0,100,11,1,1,'I~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		// $sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'type','quotes',1,'1','type','Type',1,0,0,100,12,1,1,'V~O')";
		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'tax','quotes',1,'1','txtTax','Tax',1,0,0,100,13,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'adjustment','quotes',1,'1','txtAdjustment','Adjustment',1,0,0,100,20,1,3,'NN~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'total','quotes',1,'1','hdnGrandTotal','Total',1,0,0,100,14,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'accountid','quotes',1,'73','account_id','Account Name',1,0,0,100,16,1,1,'I~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'smownerid','crmentity',1,'52','assigned_user_id','Assigned To',1,0,0,100,17,1,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'createdtime','crmentity',1,'70','createdtime','Created Time',1,0,0,100,18,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'modifiedtime','crmentity',1,'70','modifiedtime','Modified Time',1,0,0,100,19,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block 2
		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'bill_street','quotesbillads',1,'24','bill_street','Billing Address',1,0,0,100,1,2,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'ship_street','quotesshipads',1,'24','ship_street','Shipping Address',1,0,0,100,2,2,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'bill_city','quotesbillads',1,'1','bill_city','Billing City',1,0,0,100,3,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'ship_city','quotesshipads',1,'1','ship_city','Shipping City',1,0,0,100,4,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'bill_state','quotesbillads',1,'1','bill_state','Billing State',1,0,0,100,5,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'ship_state','quotesshipads',1,'1','ship_state','Shipping State',1,0,0,100,6,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'bill_code','quotesbillads',1,'1','bill_code','Billing Code',1,0,0,100,7,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'ship_code','quotesshipads',1,'1','ship_code','Shipping Code',1,0,0,100,8,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'bill_country','quotesbillads',1,'1','bill_country','Billing Country',1,0,0,100,9,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'ship_country','quotesshipads',1,'1','ship_country','Shipping Country',1,0,0,100,10,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block3
		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'description','crmentity',1,'19','description','Description',1,0,0,100,1,3,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block 6
		$sql172="insert into field values (20,".GETUNIQUEID($conn,"field").",'terms_conditions','quotes',1,'19','terms_conditions','Terms & Conditions',1,0,0,100,1,6,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'subject','purchaseorder',1,'2','subject','Subject',1,0,0,100,1,1,1,'V~M')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'vendorid','purchaseorder',1,'81','vendor_id','Vendor Name',1,0,0,100,3,1,1,'I~M')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'requisition_no','purchaseorder',1,'1','requisition_no','Requisition No',1,0,0,100,4,1,1,'V~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'tracking_no','purchaseorder',1,'1','tracking_no','Tracking Number',1,0,0,100,5,1,1,'V~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'contactid','purchaseorder',1,'57','contact_id','Contact Name',1,0,0,100,6,1,1,'I~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'duedate','purchaseorder',1,'5','duedate','Due Date',1,0,0,100,7,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'carrier','purchaseorder',1,'15','carrier','Carrier',1,0,0,100,8,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'salestax','purchaseorder',1,'1','txtTax','Sales Tax',1,0,0,100,10,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'adjustment','purchaseorder',1,'1','txtAdjustment','Adjustment',1,0,0,100,10,1,3,'NN~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'salescommission','purchaseorder',1,'1','salescommission','Sales Commission',1,0,0,100,11,1,1,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'exciseduty','purchaseorder',1,'1','exciseduty','Excise Duty',1,0,0,100,12,1,1,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'total','purchaseorder',1,'1','hdnGrandTotal','Total',1,0,0,100,13,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'subtotal','purchaseorder',1,'1','hdnSubTotal','Sub Total',1,0,0,100,14,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'postatus','purchaseorder',1,'15','postatus','Status',1,0,0,100,15,1,1,'V~O')";
		echo '<br>DON '.$sql172 .' <br> ';
		mysql_query($sql172);


		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'smownerid','crmentity',1,'52','assigned_user_id','Assigned To',1,0,0,100,16,1,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'createdtime','crmentity',1,'70','createdtime','Created Time',1,0,0,100,17,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'modifiedtime','crmentity',1,'70','modifiedtime','Modified Time',1,0,0,100,18,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block 2
		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'bill_street','pobillads',1,'24','bill_street','Billing Address',1,0,0,100,1,2,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'ship_street','poshipads',1,'24','ship_street','Shipping Address',1,0,0,100,2,2,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'bill_city','pobillads',1,'1','bill_city','Billing City',1,0,0,100,3,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'ship_city','poshipads',1,'1','ship_city','Shipping City',1,0,0,100,4,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'bill_state','pobillads',1,'1','bill_state','Billing State',1,0,0,100,5,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'ship_state','poshipads',1,'1','ship_state','Shipping State',1,0,0,100,6,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'bill_code','pobillads',1,'1','bill_code','Billing Code',1,0,0,100,7,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'ship_code','poshipads',1,'1','ship_code','Shipping Code',1,0,0,100,8,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'bill_country','pobillads',1,'1','bill_country','Billing Country',1,0,0,100,9,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'ship_country','poshipads',1,'1','ship_country','Shipping Country',1,0,0,100,10,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block3
		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'description','crmentity',1,'19','description','Description',1,0,0,100,1,3,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block6
		$sql172="insert into field values (21,".GETUNIQUEID($conn,"field").",'terms_conditions','purchaseorder',1,'19','terms_conditions','Terms & Conditions',1,0,0,100,1,6,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'subject','salesorder',1,'2','subject','Subject',1,0,0,100,1,1,1,'V~M')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'potentialid','salesorder',1,'76','potential_id','Potential Name',1,0,0,100,2,1,1,'I~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'customerno','salesorder',1,'1','customerno','Customer No',1,0,0,100,3,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'quoteid','salesorder',1,'78','quote_id','Quote Name',1,0,0,100,4,1,1,'I~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'purchaseorder','salesorder',1,'1','purchaseorder','Purchase Order',1,0,0,100,4,1,1,'V~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'contactid','salesorder',1,'57','contact_id','Contact Name',1,0,0,100,6,1,1,'I~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'duedate','salesorder',1,'5','duedate','Due Date',1,0,0,100,8,1,1,'D~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'carrier','salesorder',1,'15','carrier','Carrier',1,0,0,100,9,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'pending','salesorder',1,'1','pending','Pending',1,0,0,100,10,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'sostatus','salesorder',1,'15','sostatus','Status',1,0,0,100,11,1,1,'V~O')";
		echo '<br> DON'.$sql172 .' <br> ';
		mysql_query($sql172);


		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'salestax','salesorder',1,'1','txtTax','Sales Tax',1,0,0,100,12,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'adjustment','salesorder',1,'1','txtAdjustment','Sales Tax',1,0,0,100,12,1,3,'NN~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'salescommission','salesorder',1,'1','salescommission','Sales Commission',1,0,0,100,13,1,1,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'exciseduty','salesorder',1,'1','exciseduty','Excise Duty',1,0,0,100,13,1,1,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'total','salesorder',1,'1','hdnGrandTotal','Total',1,0,0,100,14,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'subtotal','salesorder',1,'1','hdnSubTotal','Total',1,0,0,100,15,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'accountid','salesorder',1,'73','account_id','Account Name',1,0,0,100,16,1,1,'I~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'smownerid','crmentity',1,'52','assigned_user_id','Assigned To',1,0,0,100,17,1,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'createdtime','crmentity',1,'70','createdtime','Created Time',1,0,0,100,18,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'modifiedtime','crmentity',1,'70','modifiedtime','Modified Time',1,0,0,100,19,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block 2
		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'bill_street','sobillads',1,'24','bill_street','Billing Address',1,0,0,100,1,2,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'ship_street','soshipads',1,'24','ship_street','Shipping Address',1,0,0,100,2,2,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'bill_city','sobillads',1,'1','bill_city','Billing City',1,0,0,100,3,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'ship_city','soshipads',1,'1','ship_city','Shipping City',1,0,0,100,4,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'bill_state','sobillads',1,'1','bill_state','Billing State',1,0,0,100,5,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'ship_state','soshipads',1,'1','ship_state','Shipping State',1,0,0,100,6,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'bill_code','sobillads',1,'1','bill_code','Billing Code',1,0,0,100,7,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'ship_code','soshipads',1,'1','ship_code','Shipping Code',1,0,0,100,8,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'bill_country','sobillads',1,'1','bill_country','Billing Country',1,0,0,100,9,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'ship_country','soshipads',1,'1','ship_country','Shipping Country',1,0,0,100,10,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block3
		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'description','crmentity',1,'19','description','Description',1,0,0,100,1,3,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block6
		$sql172="insert into field values (22,".GETUNIQUEID($conn,"field").",'terms_conditions','salesorder',1,'19','terms_conditions','Terms & Conditions',1,0,0,100,1,6,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'subject','invoice',1,'2','subject','Subject',1,0,0,100,1,1,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'salesorderid','invoice',1,'80','salesorder_id','Sales Order',1,0,0,100,2,1,1,'I~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'customerno','invoice',1,'1','customerno','Customer No',1,0,0,100,3,1,1,'V~O')";	
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		/*
		   $sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'notes','invoice',1,'1','notes','Notes',1,0,0,100,4,1,1,'V~O')";	
		   echo '<br> '.$sql172 .' <br> ';
		   $res172 = mysql_query($sql172);
		 */

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'invoicedate','invoice',1,'5','invoicedate','Invoice Date',1,0,0,100,5,1,1,'D~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'duedate','invoice',1,'5','duedate','Due Date',1,0,0,100,6,1,1,'D~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		/*
		   $sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'invoiceterms','invoice',1,'1','invoiceterms','Invoice Terms',1,0,0,100,7,1,1,'V~O')";
		   echo '<br> '.$sql172 .' <br> ';
		   $res172 = mysql_query($sql172);
		 */

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'purchaseorder','invoice',1,'1','purchaseorder','Purchase Order',1,0,0,100,8,1,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'salestax','invoice',1,'1','txtTax','Sales Tax',1,0,0,100,9,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'adjustment','invoice',1,'1','txtAdjustment','Sales Tax',1,0,0,100,9,1,3,'NN~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'salescommission','invoice',1,'1','salescommission','Sales Commission',1,0,0,10,13,1,1,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'exciseduty','invoice',1,'1','exciseduty','Excise Duty',1,0,0,100,11,1,1,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'subtotal','invoice',1,'1','hdnSubTotal','Sub Total',1,0,0,100,12,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'total','invoice',1,'1','hdnGrandTotal','Total',1,0,0,100,13,1,3,'N~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'accountid','invoice',1,'73','account_id','Account Name',1,0,0,100,14,1,1,'I~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'invoicestatus','invoice',1,'15','invoicestatus','Status',1,0,0,100,15,1,1,'V~O')";
		echo '<br> DON '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'smownerid','crmentity',1,'52','assigned_user_id','Assigned To',1,0,0,100,16,1,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'createdtime','crmentity',1,'70','createdtime','Created Time',1,0,0,100,17,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'modifiedtime','crmentity',1,'70','modifiedtime','Modified Time',1,0,0,100,18,1,2,'T~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block 2
		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'bill_street','invoicebillads',1,'24','bill_street','Billing Address',1,0,0,100,1,2,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'ship_street','invoiceshipads',1,'24','ship_street','Shipping Address',1,0,0,100,2,2,1,'V~M')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'bill_city','invoicebillads',1,'1','bill_city','Billing City',1,0,0,100,3,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'ship_city','invoiceshipads',1,'1','ship_city','Shipping City',1,0,0,100,4,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'bill_state','invoicebillads',1,'1','bill_state','Billing State',1,0,0,100,5,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'ship_state','invoiceshipads',1,'1','ship_state','Shipping State',1,0,0,100,6,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'bill_code','invoicebillads',1,'1','bill_code','Billing Code',1,0,0,100,7,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'ship_code','invoiceshipads',1,'1','ship_code','Shipping Code',1,0,0,100,8,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);



		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'bill_country','invoicebillads',1,'1','bill_country','Billing Country',1,0,0,100,9,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'ship_country','invoiceshipads',1,'1','ship_country','Shipping Country',1,0,0,100,10,2,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);


		//Block3
		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'description','crmentity',1,'19','description','Description',1,0,0,100,1,3,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);

		//Block6
		$sql172="insert into field values (23,".GETUNIQUEID($conn,"field").",'terms_conditions','invoice',1,'19','terms_conditions','Terms & Conditions',1,0,0,100,1,6,1,'V~O')";
		echo '<br> '.$sql172 .' <br> ';
		$res172 = mysql_query($sql172);
		...
		$sql172="insert into field values (14,".GETUNIQUEID($conn,"field").",'contactid','products',1,'57','contact_id','Contact Name',1,0,0,100,11,1,1,'I~O')";
		...
		mysql_query("insert into emailtemplates(foldername,templatename,subject,description,body,deleted,templateid) values ('Public','Announcement for Release','Announcement for Release','Announcement of a release','".$body."',0,".GETUNIQUEID($conn,'emailtemplates').")");
		...
		mysql_query("insert into emailtemplates(foldername,templatename,subject,description,body,deleted,templateid) values ('Public','Pending Invoices','Invoices Pending','Payment Due','".$body."',0,".GETUNIQUEID($conn,'emailtemplates').")");
		...
		mysql_query("insert into emailtemplates(foldername,templatename,subject,description,body,deleted,templateid) values ('Public','Acceptance Proposal','Acceptance Proposal','Acceptance of Proposal','".$body."',0,".GETUNIQUEID($conn,'emailtemplates').")");
		...
		mysql_query("insert into emailtemplates(foldername,templatename,subject,description,body,deleted,templateid) values ('Public','Good received acknowledgement','Goods received acknowledgement','Acknowledged Receipt of Goods','".$body."',0,".GETUNIQUEID($conn,'emailtemplates').")");
		...
		mysql_query("insert into emailtemplates(foldername,templatename,subject,description,body,deleted,templateid) values ('Public','Accept Order','Accept Order','Acknowledgement/Acceptance of Order','".$body."',0,".GETUNIQUEID($conn,'emailtemplates').")");
		...
		mysql_query("insert into emailtemplates(foldername,templatename,subject,description,body,deleted,templateid) values ('Public','Address Change','Change of Address','Address Change','".$body."',0,".GETUNIQUEID($conn,'emailtemplates').")");
		...
		mysql_query("insert into emailtemplates(foldername,templatename,subject,description,body,deleted,templateid) values ('Public','Follow Up','Follow Up','Follow Up of meeting','".$body."',0,".GETUNIQUEID($conn,'emailtemplates').")");
		...
		mysql_query("insert into emailtemplates(foldername,templatename,subject,description,body,deleted,templateid) values ('Public','Target Crossed!','Target Crossed!','Fantastic Sales Spree!','".$body."',0,".GETUNIQUEID($conn,'emailtemplates').")");
		...
		mysql_query("insert into emailtemplates(foldername,templatename,subject,description,body,deleted,templateid) values ('Public','Thanks Note','Thanks Note','Note of thanks','".$body."',0,".GETUNIQUEID($conn,'emailtemplates').")");
		...
		for($i=0;$i < $count;$i++)
		{
			$prof_id = mysql_result($res35,$i,'profileid');
			//insertProfile2field($prof_id);
			$fld_result = mysql_query("select * from field where displaytype in (1,2)");
			$num_rows = mysql_num_rows($fld_result);
			for($j=0; $j<$num_rows; $j++)
			{
				$tab_id = mysql_result($fld_result,$j,'tabid');
				$field_id = mysql_result($fld_result,$j,'fieldid');
				$query = "insert into profile2field values (".$prof_id.",".$tab_id.",".$field_id.",0,1)";
				echo '<br> ***********  the query is ' .$query;
				mysql_query("insert into profile2field values (".$prof_id.",".$tab_id.",".$field_id.",0,1)");
			}
		...
		{
			$tab_id = mysql_result($fld_result,$i,'tabid');
			$field_id = mysql_result($fld_result,$i,'fieldid');
			mysql_query("insert into def_org_field values (".$tab_id.",".$field_id.",0,1)");
		}
